...
ARCHITECTURE rtl OF head_ip_addr_decoder IS
  ...
  SIGNAL decode_sel_RD                    : std_logic;  -- ufix1
  SIGNAL reg_enb_RD                       : std_logic;  -- ufix1
  SIGNAL data_in_RD                       : std_logic;  -- ufix1
  SIGNAL write_reg_RD                     : std_logic;  -- ufix1
...
BEGIN
  ...
  decode_sel_RD <= '1' WHEN addr_sel_unsigned = to_unsigned(16#0042#, 14) ELSE
      '0';

  reg_enb_RD <= decode_sel_RD AND wr_enb;

  data_in_RD <= data_write_unsigned(0);

  reg_RD_process : PROCESS (clk, reset)
  BEGIN
    IF reset = '1' THEN
      write_reg_RD <= '0';
    ELSIF clk'EVENT AND clk = '1' THEN
      IF enb = '1' AND reg_enb_RD = '1' THEN
        write_reg_RD <= data_in_RD;
      END IF;
    END IF;
  END PROCESS reg_RD_process;


  write_RD <= write_reg_RD;
