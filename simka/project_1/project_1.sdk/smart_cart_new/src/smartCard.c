...
unsigned  read_serial_number(char sim_number)// функция запроса серийного номера
{
	...
	write_byte(&ccomanda,5,&cinterfacesmartcard, sim_number); //Передача данных на смарт карту
...
unsigned create_kf(char sim_number)
{
	...
	cinterfacesmartcard.transmit_buffer[2]=0x10; //арг 1
	...
	cinterfacesmartcard.transmit_buffer[0]=10; //арг 3
	write_byte(&ccomanda,1,&cinterfacesmartcard, sim_number);
	while(cinterfacesmartcard.ok==0){}
	cinterfacesmartcard.ok=0;

	cinterfacesmartcard.transmit_buffer[0]=0x00;//
	cinterfacesmartcard.transmit_buffer[1]=0x00;//Кончились
	cinterfacesmartcard.transmit_buffer[2]=0x0A; //10 попыток предъяв пароля/аутентификации
	cinterfacesmartcard.transmit_buffer[3]=0x0A; //
	...
	cinterfacesmartcard.transmit_buffer[8]=0x00;//условия доступа
	cinterfacesmartcard.transmit_buffer[9]=0x00;//
	write_byte(&ccomanda,10,&cinterfacesmartcard, sim_number);
	while(cinterfacesmartcard.ok==0){}
	cinterfacesmartcard.ok=0;
