/*
 * smartCard.c
 *
 *  Created on: 14 сент. 2016 г.
 *      Author: Admin
 */

#include "smartCard.h"

#include <stdlib.h>

unsigned intcial()
{

	int statusReg;
	u32 address;
	ProgSmartCart pProgSmartCart;
	ProgSmartCart_1 pProgSmartCart_1;
	statusReg=ScuGicInterrupt_Init(0,&pProgSmartCart,&pProgSmartCart_1);
	clean_cInterfaceSmartcard(&cinterfacesmartcard);// обнуление структуры cInterfaceSmartcard

	Xil_Out32(XPAR_HEAD_IP_1_BASEADDR+0x4,0x1);
	Xil_Out32(XPAR_HEAD_IP_0_BASEADDR+0x4,0x1);

	Xil_Out32(XPAR_HEAD_IP_0_BASEADDR+0x100,0x4);
	Xil_Out32(XPAR_HEAD_IP_1_BASEADDR+0x100,0x4);

	Xil_Out32(XPAR_HEAD_IP_1_BASEADDR+0x228,250000000);
	Xil_Out32(XPAR_HEAD_IP_0_BASEADDR+0x228,250000000);

	Xil_Out32(XPAR_HEAD_IP_0_BASEADDR+0x230,2604);
	Xil_Out32(XPAR_HEAD_IP_1_BASEADDR+0x230,2604);

	Xil_Out32(XPAR_HEAD_IP_0_BASEADDR+0x22C, 5208);
	Xil_Out32(XPAR_HEAD_IP_1_BASEADDR+0x22C, 5208);

	Xil_Out32(XPAR_HEAD_IP_0_BASEADDR+0x234,5208*2-100);
	Xil_Out32(XPAR_HEAD_IP_1_BASEADDR+0x234,5208*2-100);

	Xil_Out32(XPAR_HEAD_IP_0_BASEADDR+0x204,130);
	Xil_Out32(XPAR_HEAD_IP_1_BASEADDR+0x204,130);

	return statusReg;
}

/**
 * param sim_number - Номер смарт карты 0 - большая, 1 - маленькая.
 * */
char*  read_serial_number(int numer_sim)// функция запроса серийного номера
{
	printf("=========================\n");
	printf("Start \"read_serial_number\": \n");

	Ccomanda ccomanda;

	char* byt;

	//APDU шапка
	cinterfacesmartcard.transmit_buffer[0]=0x80; // Номер класса
	cinterfacesmartcard.transmit_buffer[1]=0xCA; // Номер инструкции
	cinterfacesmartcard.transmit_buffer[2]=0x00; // Аргумент 1
	cinterfacesmartcard.transmit_buffer[3]=0x00; // Аргумент 2
	cinterfacesmartcard.transmit_buffer[4]=0x04; // Аргумент 3

	write_byte(&ccomanda,5,&cinterfacesmartcard,numer_sim);
	while(cinterfacesmartcard.ok==0){}
    cinterfacesmartcard.ok=0;

    if(cinterfacesmartcard.reciver_buffer[0] == cinterfacesmartcard.transmit_buffer[1])
    {
       printf("Loading comand \"read_serial_number\"... \n");

       if(cinterfacesmartcard.reciver_buffer[5] == 0x90)
       {
    	   if(cinterfacesmartcard.reciver_buffer[6] == 0x00)
    	   {
    		   printf("card serial num: \n");

    		   int i =0;

    		   for(i=1; i < 5;i++)
    		   {
    		      printf("0x%02hhx\n",cinterfacesmartcard.reciver_buffer[i]);
    		   }

    		   printf("End comand \"read_serial_number\" : Good = 90 00 \n");
    	   }
       }


    }
    else
    {
    	if(cinterfacesmartcard.reciver_buffer[0] == 0x6D)
    	{
    	   if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
    	   {
    	    	printf("Error comand \"read_serial_number\" : 6D 00 (Неизвестная команда) \n");
    	    }
    	}
    }

    printf("=========================\n");


    byt[0]=cinterfacesmartcard.reciver_buffer[1];
    byt[1]=cinterfacesmartcard.reciver_buffer[2];
    byt[2]=cinterfacesmartcard.reciver_buffer[3];
    byt[3]=cinterfacesmartcard.reciver_buffer[4];

	return byt;
}


unsigned save_div_key(char tk_1, char tk_2, int numer_sim)
{
	printf("=========================\n");
		printf("Start \"save_div_key\": \n");
		printf("Go id: 0x%02hhx 0x%02hhx \n",tk_1,tk_2);

		int somo_test,i;
			Ccomanda ccomanda;
			char byt[4];
			unsigned *qwd_byt;
			qwd_byt=(unsigned*)(byt);


			cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
			cinterfacesmartcard.transmit_buffer[1]=0x56; //передаем номер инстртукции
			cinterfacesmartcard.transmit_buffer[2]=0x00; //ар
			cinterfacesmartcard.transmit_buffer[3]=0x00;

			write_byte(&ccomanda,4,&cinterfacesmartcard,numer_sim);
			while(cinterfacesmartcard.ok==0){}
			cinterfacesmartcard.ok=0;

			cinterfacesmartcard.transmit_buffer[0]=0x02;
			write_byte(&ccomanda,1,&cinterfacesmartcard,numer_sim);
			while(cinterfacesmartcard.ok==0){}
			cinterfacesmartcard.ok=0;


			if(cinterfacesmartcard.reciver_buffer[0] == cinterfacesmartcard.transmit_buffer[1])
			{
			   printf("Loading comand \"save_div_key\"... \n");
			}
			else
			{
				if(cinterfacesmartcard.reciver_buffer[0] == 0x6D)
				{
				   if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
				   {
				    	printf("Error comand \"save_div_key\" : 6D 00 (Неизвестная команда) \n");
				    }
				}
			}

			cinterfacesmartcard.transmit_buffer[0]=tk_1; //3F
			cinterfacesmartcard.transmit_buffer[1]=tk_2; // 00
			write_byte(&ccomanda,2,&cinterfacesmartcard,numer_sim);
			while(cinterfacesmartcard.ok==0){}
			cinterfacesmartcard.ok=0;

			if(cinterfacesmartcard.reciver_buffer[0] == 0x90)
			{
				if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
				{
					printf("End comand \"save_div_key\" : Good = 90 00 \n");
				}
			}
			else
			{
				if(cinterfacesmartcard.reciver_buffer[0] == 0x63)
										{
											printf("Ошибка аутентификации или предъявления пароля: %d - число оставшихся попыток \n",cinterfacesmartcard.reciver_buffer[1]);
										}

										if(cinterfacesmartcard.reciver_buffer[0] == 0x69)
										{
											switch (cinterfacesmartcard.reciver_buffer[1])
											{
												case 0x82:
														{
															printf("Доступ не разрешен или ошибка предъявления транспортного кода \n : 69 82");
															break;
														}

												case 0x83:
														{
															printf("Ключ, пароль или транспортный код блокирован \n : 69 83");
															break;
														}
												case 0x85:
														{
															printf("Не выполнены условия использования команды (не выполнялись команды GET_CHALLENGE, GEN_SESSION_KEY, нет данных для GET_RESPONSE) \n : 69 85");
															break;
														}
												case 0x86:
														{
															printf("Команда не применима к данному типу файла \n : 69 86");
															break;
														}
												case 0xF0:
														{
															printf("Операция запрещена для данного ключа \n : 69 F0");
															break;
														}
												case 0xF2:
														{
															printf("В данной сессии не выработан диверсифицированный ключ \n : 69 F2");
															break;
														}

											}
										}

										if(cinterfacesmartcard.reciver_buffer[0] == 0x6A)
										{
											switch (cinterfacesmartcard.reciver_buffer[1])
											{
												case 0x83:
												{
													printf("Файл пуст \n : 6A 83");
													break;
												}
												case 0x86:
												{
													printf("Не верные параметры Р1 Р2 \n : 6A 86");
													break;
												}
												case 0x84:
												{
													printf("Не достаточно места для создания файла \n : 6A 84");
													break;
												}
												case 0x80:
												{
													printf("Неприемлемое значение поля данных (файл с указанным идентификатором уже присутствует) \n : 6A 83");
													break;
												}
											}
										}
			}
			printf("=========================\n");



			byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
			byt[1]=cinterfacesmartcard.reciver_buffer[2]; //
			byt[2]=cinterfacesmartcard.reciver_buffer[3]; //
			byt[3]=0; //
			somo_test=*qwd_byt;
			return somo_test;
}

unsigned diversify_key(char* key,int numer_sim)
{
	printf("=========================\n");
	printf("Start \"diversify_key\": \n");


	int serial_number;
		Ccomanda ccomanda;
		char byt[4];
		unsigned *qwd_byt;
		qwd_byt=(unsigned*)(byt);
		//APDU шапка
		cinterfacesmartcard.transmit_buffer[0]=0x80; // Номер класса
		cinterfacesmartcard.transmit_buffer[1]=0x48; // Номер инструкции
		cinterfacesmartcard.transmit_buffer[2]=0x00; // Аргумент 1
		cinterfacesmartcard.transmit_buffer[3]=0x00; // Аргумент 2
		cinterfacesmartcard.transmit_buffer[4]=0x04; // Аргумент 3
		write_byte(&ccomanda,5,&cinterfacesmartcard,numer_sim);
		while(cinterfacesmartcard.ok==0){}
	    cinterfacesmartcard.ok=0;



		if(cinterfacesmartcard.reciver_buffer[0] == cinterfacesmartcard.transmit_buffer[1])
		{
			printf("Loading comand \"diversify_key\"... \n");
		}
		else
		{
			if(cinterfacesmartcard.reciver_buffer[0] == 0x6D)
			{
				if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
				{
					printf("Error comand \"diversify_key\" : 6D 00 (Неизвестная команда) \n");
				}
			}

			if(cinterfacesmartcard.reciver_buffer[0] == 0x63)
							{
								printf("Ошибка аутентификации или предъявления пароля: %d - число оставшихся попыток \n",cinterfacesmartcard.reciver_buffer[1]);
							}

							if(cinterfacesmartcard.reciver_buffer[0] == 0x69)
							{
								switch (cinterfacesmartcard.reciver_buffer[1])
								{
									case 0x82:
											{
												printf("Доступ не разрешен или ошибка предъявления транспортного кода \n : 69 82");
												break;
											}

									case 0x83:
											{
												printf("Ключ, пароль или транспортный код блокирован \n : 69 83");
												break;
											}
									case 0x85:
											{
												printf("Не выполнены условия использования команды (не выполнялись команды GET_CHALLENGE, GEN_SESSION_KEY, нет данных для GET_RESPONSE) \n : 69 85");
												break;
											}
									case 0x86:
											{
												printf("Команда не применима к данному типу файла \n : 69 86");
												break;
											}
									case 0xF0:
											{
												printf("Операция запрещена для данного ключа \n : 69 F0");
												break;
											}
									case 0xF2:
											{
												printf("В данной сессии не выработан диверсифицированный ключ \n : 69 F2");
												break;
											}

								}
							}

							if(cinterfacesmartcard.reciver_buffer[0] == 0x6A)
							{
								switch (cinterfacesmartcard.reciver_buffer[1])
								{
									case 0x83:
									{
										printf("Файл пуст \n : 6A 83");
										break;
									}
									case 0x86:
									{
										printf("Не верные параметры Р1 Р2 \n : 6A 86");
										break;
									}
									case 0x84:
									{
										printf("Не достаточно места для создания файла \n : 6A 84");
										break;
									}
									case 0x80:
									{
										printf("Неприемлемое значение поля данных (файл с указанным идентификатором уже присутствует) \n : 6A 83");
										break;
									}
								}
							}
		}




	    cinterfacesmartcard.transmit_buffer[0]=key[0]; // Номер класса
	    cinterfacesmartcard.transmit_buffer[1]=key[1]; // Номер инструкции
	    cinterfacesmartcard.transmit_buffer[2]=key[2]; // Аргумент 1
	    cinterfacesmartcard.transmit_buffer[3]=key[3]; // Аргумент 2
	    write_byte(&ccomanda,4,&cinterfacesmartcard,numer_sim);
	    while(cinterfacesmartcard.ok==0){}
	    cinterfacesmartcard.ok=0;




	   if(cinterfacesmartcard.reciver_buffer[0] == 0x90)
	   {
	    	   if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
	    	   {
	    		   printf("End comand \"diversify_key\" : Good = 90 00 \n");
	    	   }
	    }
	    else
	    {
	    	if(cinterfacesmartcard.reciver_buffer[0] == 0x6D)
	    	{
	    	   if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
	    	   {
	    	    	printf("Error comand \"diversify_key\" : 6D 00 (Неизвестная команда) \n");
	    	    }
	    	}
	    }

	    printf("=========================\n");


		byt[0]=cinterfacesmartcard.reciver_buffer[1];
		byt[1]=cinterfacesmartcard.reciver_buffer[2];
		byt[2]=cinterfacesmartcard.reciver_buffer[3];
		byt[3]=cinterfacesmartcard.reciver_buffer[4];
		serial_number=*qwd_byt;
		return serial_number;
}
/**
 * param:
 * id - //номер KF в формате 1n , где n любое число от 0 до 9
 * AC - массив условия доступа ( состоит из 4 байт , каждый байт состоит из 2 тетрад , каждая тетрада отвечает за условие доступа к KF (читай документацию с таблицей в 16 колонок )
 * B18 - байт 18 ( максимум попыток 15) (читай документацию) , где 1 старшая тетрада отвечает за максимальное количество попыток предъявления пароля или ключа для VERIFY, а младшая тетрада указывает на оставшеся количество попыток предъявления пароля или ключа.
 * */
unsigned create_kf(char id,char* AC, char B18, char B19, int numer_sim)
{
	printf("=========================\n");
	printf("Start \"create_kf\": \n");
	printf("ID: 0xFF 0x%02hhx\n", id);
	printf("AC: \n");

	int i =0;

	for(i=0; i < 4;i++)
	{
	   printf("0x%02hhx\n",AC[i]);
	}

	printf("Byt18: 0x%02hhx (количество попыток)\n", B18);
	printf("Byt19: 0x%02hhx (условия доступа)\n", B19);

	int somo_test;
	Ccomanda ccomanda;
	char byt[4];
	unsigned *qwd_byt;
	qwd_byt=(unsigned*)(byt);

	byt[0] = AC[0];

	byt[1] = AC[1];
	byt[2] = AC[2];
	byt[3] = AC[3];


	cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
	cinterfacesmartcard.transmit_buffer[1]=0x36; //передаем номер инстртукции
	cinterfacesmartcard.transmit_buffer[2]=id; //номер KF в формате 1n
	cinterfacesmartcard.transmit_buffer[3]=0x00; //арг 2
	cinterfacesmartcard.transmit_buffer[4]=0x0a; //Lc

	write_byte(&ccomanda,5,&cinterfacesmartcard, numer_sim);
	while(cinterfacesmartcard.ok==0){}
	cinterfacesmartcard.ok=0;

	if(cinterfacesmartcard.reciver_buffer[0] == cinterfacesmartcard.transmit_buffer[1])
	{
		printf("Loading comand \"create_kf\"... \n");
	}
	else
	{
		if(cinterfacesmartcard.reciver_buffer[0] == 0x6D)
		{
			if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
			{
				printf("Error comand \"create_kf\" : 6D 00 (Неизвестная команда) \n");
			}
		}

		if(cinterfacesmartcard.reciver_buffer[0] == 0x63)
						{
							printf("Ошибка аутентификации или предъявления пароля: %d - число оставшихся попыток \n",cinterfacesmartcard.reciver_buffer[1]);
						}

						if(cinterfacesmartcard.reciver_buffer[0] == 0x69)
						{
							switch (cinterfacesmartcard.reciver_buffer[1])
							{
								case 0x82:
										{
											printf("Доступ не разрешен или ошибка предъявления транспортного кода \n : 69 82");
											break;
										}

								case 0x83:
										{
											printf("Ключ, пароль или транспортный код блокирован \n : 69 83");
											break;
										}
								case 0x85:
										{
											printf("Не выполнены условия использования команды (не выполнялись команды GET_CHALLENGE, GEN_SESSION_KEY, нет данных для GET_RESPONSE) \n : 69 85");
											break;
										}
								case 0x86:
										{
											printf("Команда не применима к данному типу файла \n : 69 86");
											break;
										}
								case 0xF0:
										{
											printf("Операция запрещена для данного ключа \n : 69 F0");
											break;
										}
								case 0xF2:
										{
											printf("В данной сессии не выработан диверсифицированный ключ \n : 69 F2");
											break;
										}

							}
						}

						if(cinterfacesmartcard.reciver_buffer[0] == 0x6A)
						{
							switch (cinterfacesmartcard.reciver_buffer[1])
							{
								case 0x83:
								{
									printf("Файл пуст \n : 6A 83");
									break;
								}
								case 0x86:
								{
									printf("Не верные параметры Р1 Р2 \n : 6A 86");
									break;
								}
								case 0x84:
								{
									printf("Не достаточно места для создания файла \n : 6A 84");
									break;
								}
								case 0x80:
								{
									printf("Неприемлемое значение поля данных (файл с указанным идентификатором уже присутствует) \n : 6A 83");
									break;
								}
							}
						}
	}


	cinterfacesmartcard.transmit_buffer[0]=AC[0];// 0x00;
	cinterfacesmartcard.transmit_buffer[1]=AC[1];// 0x0F;
	cinterfacesmartcard.transmit_buffer[2]=AC[2];// 0xF0;
	cinterfacesmartcard.transmit_buffer[3]=AC[3];//0xFF;

	cinterfacesmartcard.transmit_buffer[4]=0xFF;
	cinterfacesmartcard.transmit_buffer[5]=0xFF;
	cinterfacesmartcard.transmit_buffer[6]=0xFF;
	cinterfacesmartcard.transmit_buffer[7]=0xFF;

	cinterfacesmartcard.transmit_buffer[8]=B18;//0x50;// байт 18 ( 5 попыток предъяв пароля/аутентификации )
	cinterfacesmartcard.transmit_buffer[9]=B19; // 0x00; // байт 19

	write_byte(&ccomanda,10,&cinterfacesmartcard, numer_sim);
	while(cinterfacesmartcard.ok==0){}
	cinterfacesmartcard.ok=0;

	if(cinterfacesmartcard.reciver_buffer[0] == 0x90)
	{
		if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
		{
			printf("End comand \"create_kf\" : Good = 90 00 \n");
		}
	}
	else
	{
		if(cinterfacesmartcard.reciver_buffer[0] == 0x63)
								{
									printf("Ошибка аутентификации или предъявления пароля: %d - число оставшихся попыток \n",cinterfacesmartcard.reciver_buffer[1]);
								}

								if(cinterfacesmartcard.reciver_buffer[0] == 0x69)
								{
									switch (cinterfacesmartcard.reciver_buffer[1])
									{
										case 0x82:
												{
													printf("Доступ не разрешен или ошибка предъявления транспортного кода \n : 69 82");
													break;
												}

										case 0x83:
												{
													printf("Ключ, пароль или транспортный код блокирован \n : 69 83");
													break;
												}
										case 0x85:
												{
													printf("Не выполнены условия использования команды (не выполнялись команды GET_CHALLENGE, GEN_SESSION_KEY, нет данных для GET_RESPONSE) \n : 69 85");
													break;
												}
										case 0x86:
												{
													printf("Команда не применима к данному типу файла \n : 69 86");
													break;
												}
										case 0xF0:
												{
													printf("Операция запрещена для данного ключа \n : 69 F0");
													break;
												}
										case 0xF2:
												{
													printf("В данной сессии не выработан диверсифицированный ключ \n : 69 F2");
													break;
												}

									}
								}

								if(cinterfacesmartcard.reciver_buffer[0] == 0x6A)
								{
									switch (cinterfacesmartcard.reciver_buffer[1])
									{
										case 0x83:
										{
											printf("Файл пуст \n : 6A 83");
											break;
										}
										case 0x86:
										{
											printf("Не верные параметры Р1 Р2 \n : 6A 86");
											break;
										}
										case 0x84:
										{
											printf("Не достаточно места для создания файла \n : 6A 84");
											break;
										}
										case 0x80:
										{
											printf("Неприемлемое значение поля данных (файл с указанным идентификатором уже присутствует) \n : 6A 80");
											break;
										}
									}
								}
	}
	printf("=========================\n");


	byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
	byt[1]=cinterfacesmartcard.reciver_buffer[2]; //
	byt[3]=cinterfacesmartcard.reciver_buffer[3]; //
	byt[4]=cinterfacesmartcard.reciver_buffer[4];
	byt[5]=cinterfacesmartcard.reciver_buffer[5];
	byt[6]=cinterfacesmartcard.reciver_buffer[6];

	somo_test=*qwd_byt;
	return somo_test;

}

unsigned external_authenticate(char* kriptogramma, int numer_sim)
{
	printf("=========================\n");
	printf("Start \"external_authenticate\": \n");

	int somo_test;
	Ccomanda ccomanda;
	char byt[4];
	unsigned *qwd_byt;
	qwd_byt=(unsigned*)(byt);
	cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
	cinterfacesmartcard.transmit_buffer[1]=0x82; //передаем номер инстртукции
	cinterfacesmartcard.transmit_buffer[2]=0x00; //ар
	cinterfacesmartcard.transmit_buffer[3]=0x00;
	cinterfacesmartcard.transmit_buffer[4]=0x06;
	write_byte(&ccomanda,5,&cinterfacesmartcard,numer_sim);
	while(cinterfacesmartcard.ok==0){}
	cinterfacesmartcard.ok=0;


	if(cinterfacesmartcard.reciver_buffer[0] == cinterfacesmartcard.transmit_buffer[1])
		{
			printf("Loading comand \"external_authenticate\"... \n");
		}
		else
		{
			if(cinterfacesmartcard.reciver_buffer[0] == 0x6D)
			{
				if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
				{
					printf("Error comand \"external_authenticate\" : 6D 00 (Неизвестная команда) \n");
				}
			}

			if(cinterfacesmartcard.reciver_buffer[0] == 0x63)
							{
								printf("Ошибка аутентификации или предъявления пароля: %d - число оставшихся попыток \n",cinterfacesmartcard.reciver_buffer[1]);
							}

							if(cinterfacesmartcard.reciver_buffer[0] == 0x69)
							{
								switch (cinterfacesmartcard.reciver_buffer[1])
								{
									case 0x82:
											{
												printf("Доступ не разрешен или ошибка предъявления транспортного кода \n : 69 82");
												break;
											}

									case 0x83:
											{
												printf("Ключ, пароль или транспортный код блокирован \n : 69 83");
												break;
											}
									case 0x85:
											{
												printf("Не выполнены условия использования команды (не выполнялись команды GET_CHALLENGE, GEN_SESSION_KEY, нет данных для GET_RESPONSE) \n : 69 85");
												break;
											}
									case 0x86:
											{
												printf("Команда не применима к данному типу файла \n : 69 86");
												break;
											}
									case 0xF0:
											{
												printf("Операция запрещена для данного ключа \n : 69 F0");
												break;
											}
									case 0xF2:
											{
												printf("В данной сессии не выработан диверсифицированный ключ \n : 69 F2");
												break;
											}

								}
							}

							if(cinterfacesmartcard.reciver_buffer[0] == 0x6A)
							{
								switch (cinterfacesmartcard.reciver_buffer[1])
								{
									case 0x83:
									{
										printf("Файл пуст \n : 6A 83");
										break;
									}
									case 0x86:
									{
										printf("Не верные параметры Р1 Р2 \n : 6A 86");
										break;
									}
									case 0x84:
									{
										printf("Не достаточно места для создания файла \n : 6A 84");
										break;
									}
									case 0x80:
									{
										printf("Неприемлемое значение поля данных (файл с указанным идентификатором уже присутствует) \n : 6A 83");
										break;
									}
								}
							}
		}


	cinterfacesmartcard.transmit_buffer[0]=kriptogramma[0];
	cinterfacesmartcard.transmit_buffer[1]=kriptogramma[1];
	cinterfacesmartcard.transmit_buffer[2]=kriptogramma[2];
	cinterfacesmartcard.transmit_buffer[3]=kriptogramma[3];
	cinterfacesmartcard.transmit_buffer[4]=kriptogramma[4];
	cinterfacesmartcard.transmit_buffer[5]=kriptogramma[5];
	write_byte(&ccomanda,6,&cinterfacesmartcard,numer_sim);
	while(cinterfacesmartcard.ok==0){}
	cinterfacesmartcard.ok=0;


	if(cinterfacesmartcard.reciver_buffer[0] == 0x90)
		{
			if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
			{
				printf("End comand \"external_authenticate\" : Good = 90 00 \n");
			}
		}
		else
		{
			if(cinterfacesmartcard.reciver_buffer[0] == 0x63)
									{
										printf("Ошибка аутентификации или предъявления пароля: %d - число оставшихся попыток \n",cinterfacesmartcard.reciver_buffer[1]);
									}

									if(cinterfacesmartcard.reciver_buffer[0] == 0x69)
									{
										switch (cinterfacesmartcard.reciver_buffer[1])
										{
											case 0x82:
													{
														printf("Доступ не разрешен или ошибка предъявления транспортного кода \n : 69 82");
														break;
													}

											case 0x83:
													{
														printf("Ключ, пароль или транспортный код блокирован \n : 69 83");
														break;
													}
											case 0x85:
													{
														printf("Не выполнены условия использования команды (не выполнялись команды GET_CHALLENGE, GEN_SESSION_KEY, нет данных для GET_RESPONSE) \n : 69 85");
														break;
													}
											case 0x86:
													{
														printf("Команда не применима к данному типу файла \n : 69 86");
														break;
													}
											case 0xF0:
													{
														printf("Операция запрещена для данного ключа \n : 69 F0");
														break;
													}
											case 0xF2:
													{
														printf("В данной сессии не выработан диверсифицированный ключ \n : 69 F2");
														break;
													}

										}
									}

									if(cinterfacesmartcard.reciver_buffer[0] == 0x6A)
									{
										switch (cinterfacesmartcard.reciver_buffer[1])
										{
											case 0x83:
											{
												printf("Файл пуст \n : 6A 83");
												break;
											}
											case 0x86:
											{
												printf("Не верные параметры Р1 Р2 \n : 6A 86");
												break;
											}
											case 0x84:
											{
												printf("Не достаточно места для создания файла \n : 6A 84");
												break;
											}
											case 0x80:
											{
												printf("Неприемлемое значение поля данных (файл с указанным идентификатором уже присутствует) \n : 6A 80");
												break;
											}
										}
									}
		}
		printf("=========================\n");


	byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
	byt[1]=cinterfacesmartcard.reciver_buffer[2]; //
	byt[2]=cinterfacesmartcard.reciver_buffer[3]; //
	byt[2]=cinterfacesmartcard.reciver_buffer[4];
	byt[2]=cinterfacesmartcard.reciver_buffer[5];
	byt[2]=cinterfacesmartcard.reciver_buffer[6];
	somo_test=*qwd_byt;
	return somo_test;
}

char* internal_auth(char* random, char sim_number)
{
	printf("=========================\n");
	printf("Start \"internal_auth\": \n");

	int somo_test;
	Ccomanda ccomanda;
	char byt[4];
	unsigned *qwd_byt;
	qwd_byt=(unsigned*)(byt);
	cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
	cinterfacesmartcard.transmit_buffer[1]=0x88; //передаем номер инстртукции
	cinterfacesmartcard.transmit_buffer[2]=0x00; //ар
	cinterfacesmartcard.transmit_buffer[3]=0x00;

	write_byte(&ccomanda,4,&cinterfacesmartcard,sim_number);
	while(cinterfacesmartcard.ok==0){}
	cinterfacesmartcard.ok=0;

	cinterfacesmartcard.transmit_buffer[0]=0x08;
	write_byte(&ccomanda,1,&cinterfacesmartcard,sim_number);
	while(cinterfacesmartcard.ok==0){}
	cinterfacesmartcard.ok=0;


	if(cinterfacesmartcard.reciver_buffer[0] == cinterfacesmartcard.transmit_buffer[1])
		{
			printf("Loading comand \"internal_auth\"... \n");
		}
		else
		{
			if(cinterfacesmartcard.reciver_buffer[0] == 0x6D)
			{
				if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
				{
					printf("Error comand \"internal_auth\" : 6D 00 (Неизвестная команда) \n");
				}
			}

			if(cinterfacesmartcard.reciver_buffer[0] == 0x63)
							{
								printf("Ошибка аутентификации или предъявления пароля: %d - число оставшихся попыток \n",cinterfacesmartcard.reciver_buffer[1]);
							}

							if(cinterfacesmartcard.reciver_buffer[0] == 0x69)
							{
								switch (cinterfacesmartcard.reciver_buffer[1])
								{
									case 0x82:
											{
												printf("Доступ не разрешен или ошибка предъявления транспортного кода \n : 69 82");
												break;
											}

									case 0x83:
											{
												printf("Ключ, пароль или транспортный код блокирован \n : 69 83");
												break;
											}
									case 0x85:
											{
												printf("Не выполнены условия использования команды (не выполнялись команды GET_CHALLENGE, GEN_SESSION_KEY, нет данных для GET_RESPONSE) \n : 69 85");
												break;
											}
									case 0x86:
											{
												printf("Команда не применима к данному типу файла \n : 69 86");
												break;
											}
									case 0xF0:
											{
												printf("Операция запрещена для данного ключа \n : 69 F0");
												break;
											}
									case 0xF2:
											{
												printf("В данной сессии не выработан диверсифицированный ключ \n : 69 F2");
												break;
											}

								}
							}

							if(cinterfacesmartcard.reciver_buffer[0] == 0x6A)
							{
								switch (cinterfacesmartcard.reciver_buffer[1])
								{
									case 0x83:
									{
										printf("Файл пуст \n : 6A 83");
										break;
									}
									case 0x86:
									{
										printf("Не верные параметры Р1 Р2 \n : 6A 86");
										break;
									}
									case 0x84:
									{
										printf("Не достаточно места для создания файла \n : 6A 84");
										break;
									}
									case 0x80:
									{
										printf("Неприемлемое значение поля данных (файл с указанным идентификатором уже присутствует) \n : 6A 83");
										break;
									}
								}
							}
		}


	cinterfacesmartcard.transmit_buffer[0]=random[0];
	cinterfacesmartcard.transmit_buffer[1]=random[1];
	cinterfacesmartcard.transmit_buffer[2]=random[2];
	cinterfacesmartcard.transmit_buffer[3]=random[3];
	cinterfacesmartcard.transmit_buffer[4]=random[4];
	cinterfacesmartcard.transmit_buffer[5]=random[5];
	cinterfacesmartcard.transmit_buffer[6]=random[6];
	cinterfacesmartcard.transmit_buffer[7]=random[7];

	write_byte(&ccomanda,8,&cinterfacesmartcard,sim_number);
	while(cinterfacesmartcard.ok==0){}
	cinterfacesmartcard.ok=0;

	if(cinterfacesmartcard.reciver_buffer[0] == 0x90)
			{
				if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
				{
					printf("End comand \"internal_auth\" : Good = 90 00 \n");
				}
			}
			else
			{
				if(cinterfacesmartcard.reciver_buffer[0] == 0x63)
										{
											printf("Ошибка аутентификации или предъявления пароля: %d - число оставшихся попыток \n",cinterfacesmartcard.reciver_buffer[1]);
										}

										if(cinterfacesmartcard.reciver_buffer[0] == 0x69)
										{
											switch (cinterfacesmartcard.reciver_buffer[1])
											{
												case 0x82:
														{
															printf("Доступ не разрешен или ошибка предъявления транспортного кода \n : 69 82");
															break;
														}

												case 0x83:
														{
															printf("Ключ, пароль или транспортный код блокирован \n : 69 83");
															break;
														}
												case 0x85:
														{
															printf("Не выполнены условия использования команды (не выполнялись команды GET_CHALLENGE, GEN_SESSION_KEY, нет данных для GET_RESPONSE) \n : 69 85");
															break;
														}
												case 0x86:
														{
															printf("Команда не применима к данному типу файла \n : 69 86");
															break;
														}
												case 0xF0:
														{
															printf("Операция запрещена для данного ключа \n : 69 F0");
															break;
														}
												case 0xF2:
														{
															printf("В данной сессии не выработан диверсифицированный ключ \n : 69 F2");
															break;
														}

											}
										}

										if(cinterfacesmartcard.reciver_buffer[0] == 0x6A)
										{
											switch (cinterfacesmartcard.reciver_buffer[1])
											{
												case 0x83:
												{
													printf("Файл пуст \n : 6A 83");
													break;
												}
												case 0x86:
												{
													printf("Не верные параметры Р1 Р2 \n : 6A 86");
													break;
												}
												case 0x84:
												{
													printf("Не достаточно места для создания файла \n : 6A 84");
													break;
												}
												case 0x80:
												{
													printf("Неприемлемое значение поля данных (файл с указанным идентификатором уже присутствует) \n : 6A 80");
													break;
												}
											}
										}
			}
			printf("=========================\n");


	byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
	byt[1]=cinterfacesmartcard.reciver_buffer[2]; //
	byt[2]=cinterfacesmartcard.reciver_buffer[3]; //
	byt[2]=cinterfacesmartcard.reciver_buffer[4];
	byt[2]=cinterfacesmartcard.reciver_buffer[5];
	byt[2]=cinterfacesmartcard.reciver_buffer[6];
	somo_test=*qwd_byt;
	return somo_test;
}


unsigned create_ef(char* size,char* id,char AC, char B18,int numer_sim)
{

	char lol[2];

	lol[0] = size[0];
	lol[1] = size[1];


	char lol2[2];

		lol2[0] = id[0];
		lol2[1] = id[1];


	printf("=========================\n");
	printf("Start \"create_ef\": \n");
	printf("ID: 0x%02hhx 0x%02hhx\n", id[0],id[1]);
	printf("size: 0x%02hhx 0x%02hhx (в байтах)\n", size[0],size[1]);
	printf("Byt18: 0x%02hhx (байт статуса)\n",B18);
	printf("AC: \n");
	printf("0x%02hhx\n",AC);
	int i =0;

	for(i=0; i < 7;i++)
	{
		printf("0xFF\n");
	}



	int somo_test;
		Ccomanda ccomanda;
		char byt[4];
		unsigned *qwd_byt;
		qwd_byt=(unsigned*)(byt);
		cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
		cinterfacesmartcard.transmit_buffer[1]=0x34; //передаем номер инстртукции
		cinterfacesmartcard.transmit_buffer[2]=0x00; //ар
		cinterfacesmartcard.transmit_buffer[3]=0x00; //гу
		cinterfacesmartcard.transmit_buffer[4]=0x0d;


		write_byte(&ccomanda,5,&cinterfacesmartcard,0);
		while(cinterfacesmartcard.ok==0){}
		cinterfacesmartcard.ok=0;


		if(cinterfacesmartcard.reciver_buffer[0] == cinterfacesmartcard.transmit_buffer[1])
		{
			printf("Loading comand \"create_ef\"... \n");
		}
		else
		{
			if(cinterfacesmartcard.reciver_buffer[0] == 0x6D)
			{
				if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
				{
					printf("Error comand \"create_ef\" : 6D 00 (Неизвестная команда) \n");
				}
			}

			if(cinterfacesmartcard.reciver_buffer[0] == 0x69)
						{
							if(cinterfacesmartcard.reciver_buffer[1] == 0x82)
							{
								printf("Доступ не разрешен или ошибка предъявления транспортного кода \"create_ef\" : 69 82  \n");
							}
						}
		}


		cinterfacesmartcard.transmit_buffer[0]=size[0]; //размер файла
		cinterfacesmartcard.transmit_buffer[1]=size[1]; //размер файла

		write_byte(&ccomanda,2,&cinterfacesmartcard,numer_sim);
		while(cinterfacesmartcard.ok==0){}
		cinterfacesmartcard.ok=0;


		cinterfacesmartcard.transmit_buffer[0]=id[0]; // индификатор файла
		cinterfacesmartcard.transmit_buffer[1]=id[1]; // индификатор файла

		write_byte(&ccomanda,2,&cinterfacesmartcard,numer_sim);
			while(cinterfacesmartcard.ok==0){}
			cinterfacesmartcard.ok=0;


		cinterfacesmartcard.transmit_buffer[0]=AC; // индификатор файла
		cinterfacesmartcard.transmit_buffer[1]=0x00; // индификатор файла


		write_byte(&ccomanda,2,&cinterfacesmartcard,numer_sim);
						while(cinterfacesmartcard.ok==0){}
						cinterfacesmartcard.ok=0;

		cinterfacesmartcard.transmit_buffer[0]=0x00;
		cinterfacesmartcard.transmit_buffer[1]=0x00;

		write_byte(&ccomanda,2,&cinterfacesmartcard,numer_sim);
		while(cinterfacesmartcard.ok==0){}
		cinterfacesmartcard.ok=0;

		cinterfacesmartcard.transmit_buffer[0]=0x00;
		cinterfacesmartcard.transmit_buffer[1]=0x00;

		write_byte(&ccomanda,2,&cinterfacesmartcard,numer_sim);
		while(cinterfacesmartcard.ok==0){}
		cinterfacesmartcard.ok=0;

		cinterfacesmartcard.transmit_buffer[0]=0x00;
		cinterfacesmartcard.transmit_buffer[1]=0x00;
		write_byte(&ccomanda,2,&cinterfacesmartcard,numer_sim);
		while(cinterfacesmartcard.ok==0){}
		cinterfacesmartcard.ok=0;

		//write_byte(&ccomanda,8,&cinterfacesmartcard,numer_sim);
			//	while(cinterfacesmartcard.ok==0){}
			//	cinterfacesmartcard.ok=0;

		cinterfacesmartcard.transmit_buffer[0]=B18;

		write_byte(&ccomanda,1,&cinterfacesmartcard,numer_sim);
		while(cinterfacesmartcard.ok==0){}
		cinterfacesmartcard.ok=0;


		if(cinterfacesmartcard.reciver_buffer[0] == 0x90)
		{
			if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
			{
				printf("End comand \"create_df\" : Good = 90 00 \n");
			}
		}
		else
		{
			if(cinterfacesmartcard.reciver_buffer[0] == 0x63)
									{
										printf("Ошибка аутентификации или предъявления пароля: %d - число оставшихся попыток \n",cinterfacesmartcard.reciver_buffer[1]);
									}

									if(cinterfacesmartcard.reciver_buffer[0] == 0x69)
									{
										switch (cinterfacesmartcard.reciver_buffer[1])
										{
											case 0x82:
													{
														printf("Доступ не разрешен или ошибка предъявления транспортного кода \n : 69 82");
														break;
													}

											case 0x83:
													{
														printf("Ключ, пароль или транспортный код блокирован \n : 69 83");
														break;
													}
											case 0x85:
													{
														printf("Не выполнены условия использования команды (не выполнялись команды GET_CHALLENGE, GEN_SESSION_KEY, нет данных для GET_RESPONSE) \n : 69 85");
														break;
													}
											case 0x86:
													{
														printf("Команда не применима к данному типу файла \n : 69 86");
														break;
													}
											case 0xF0:
													{
														printf("Операция запрещена для данного ключа \n : 69 F0");
														break;
													}
											case 0xF2:
													{
														printf("В данной сессии не выработан диверсифицированный ключ \n : 69 F2");
														break;
													}

										}
									}

									if(cinterfacesmartcard.reciver_buffer[0] == 0x6A)
									{
										switch (cinterfacesmartcard.reciver_buffer[1])
										{
											case 0x83:
											{
												printf("Файл пуст \n : 6A 83");
												break;
											}
											case 0x86:
											{
												printf("Не верные параметры Р1 Р2 \n : 6A 86");
												break;
											}
											case 0x84:
											{
												printf("Не достаточно места для создания файла \n : 6A 84");
												break;
											}
											case 0x80:
											{
												printf("Неприемлемое значение поля данных (файл с указанным идентификатором уже присутствует) \n : 6A 83");
												break;
											}
										}
									}
		}
		printf("=========================\n");


		byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
		byt[1]=cinterfacesmartcard.reciver_buffer[2]; //
		byt[2]=cinterfacesmartcard.reciver_buffer[3]; //
		byt[3]=0; //
		somo_test=*qwd_byt;
		return somo_test;
}

/*
unsigned check_rom(char sim_number)
{
	int somo_test;
	Ccomanda ccomanda;
	char byt[4];
	unsigned *qwd_byt;
	qwd_byt=(unsigned*)(byt);
	cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
	cinterfacesmartcard.transmit_buffer[1]=0x52; //передаем номер инстртукции
	cinterfacesmartcard.transmit_buffer[2]=0x00; //ар
	cinterfacesmartcard.transmit_buffer[3]=0x00;

	write_byte(&ccomanda,4,&cinterfacesmartcard, sim_number);
	while(cinterfacesmartcard.ok==0){}
	cinterfacesmartcard.ok=0;

	cinterfacesmartcard.transmit_buffer[0]=0x20;
	write_byte(&ccomanda,1,&cinterfacesmartcard, sim_number);
	while(cinterfacesmartcard.ok==0){} ///
	cinterfacesmartcard.ok=0;


	cinterfacesmartcard.transmit_buffer[0]=0x00;
	cinterfacesmartcard.transmit_buffer[1]=0x00;
	cinterfacesmartcard.transmit_buffer[2]=0x00;
	cinterfacesmartcard.transmit_buffer[3]=0x11;
	cinterfacesmartcard.transmit_buffer[4]=0x11;
	cinterfacesmartcard.transmit_buffer[5]=0x11;
	cinterfacesmartcard.transmit_buffer[6]=0x11;
	cinterfacesmartcard.transmit_buffer[7]=0x00;
	cinterfacesmartcard.transmit_buffer[8]=0x00;
	cinterfacesmartcard.transmit_buffer[9]=0x00;
	cinterfacesmartcard.transmit_buffer[10]=0x00;
	cinterfacesmartcard.transmit_buffer[11]=0x00;
	cinterfacesmartcard.transmit_buffer[12]=0x00;
	cinterfacesmartcard.transmit_buffer[13]=0x00;
	cinterfacesmartcard.transmit_buffer[14]=0x00;
	cinterfacesmartcard.transmit_buffer[15]=0x00;
	cinterfacesmartcard.transmit_buffer[16]=0x00;
	cinterfacesmartcard.transmit_buffer[17]=0x00;
	cinterfacesmartcard.transmit_buffer[18]=0x00;
	cinterfacesmartcard.transmit_buffer[19]=0x00;
	cinterfacesmartcard.transmit_buffer[20]=0x00;
	cinterfacesmartcard.transmit_buffer[21]=0x00;
	cinterfacesmartcard.transmit_buffer[22]=0x00;
	cinterfacesmartcard.transmit_buffer[23]=0x00;
	cinterfacesmartcard.transmit_buffer[24]=0x00;
	cinterfacesmartcard.transmit_buffer[25]=0x00;
	cinterfacesmartcard.transmit_buffer[26]=0x00;
	cinterfacesmartcard.transmit_buffer[27]=0x00;
	cinterfacesmartcard.transmit_buffer[28]=0x00;
	cinterfacesmartcard.transmit_buffer[29]=0x00;
	cinterfacesmartcard.transmit_buffer[30]=0x00;
	cinterfacesmartcard.transmit_buffer[31]=0x00;

	write_byte(&ccomanda,32,&cinterfacesmartcard, sim_number);
	while(cinterfacesmartcard.ok==0){}
	cinterfacesmartcard.ok=0;

	cinterfacesmartcard.transmit_buffer[0]=0x04;
	write_byte(&ccomanda,1,&cinterfacesmartcard, sim_number);
	while(cinterfacesmartcard.ok==0){}
	cinterfacesmartcard.ok=0;

	byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
	byt[1]=cinterfacesmartcard.reciver_buffer[2]; //
	byt[2]=cinterfacesmartcard.reciver_buffer[3]; //
	byt[3]=0; //
	somo_test=*qwd_byt;
	return somo_test;
}
*/

//работает
unsigned test_result(int numer_sim)// функция запроса теста системы
{
	printf("=========================\n");
	printf("Start \"test_result\": \n");
	printf("Номер симки: %d\n",numer_sim);

	unsigned somo_test;
	Ccomanda ccomanda;
	char byt[4];
	unsigned *qwd_byt;
	qwd_byt=(unsigned*)(byt);
	cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
	cinterfacesmartcard.transmit_buffer[1]=0x38; //передаем номер инстртукции
	cinterfacesmartcard.transmit_buffer[2]=0x00; //ар
	cinterfacesmartcard.transmit_buffer[3]=0x00; //гу
	cinterfacesmartcard.transmit_buffer[4]=0x03; //менты
	write_byte(&ccomanda,5,&cinterfacesmartcard,0);
	while(cinterfacesmartcard.ok==0){}
	cinterfacesmartcard.ok=0;
	byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
	byt[1]=cinterfacesmartcard.reciver_buffer[2]; //
	byt[2]=cinterfacesmartcard.reciver_buffer[3]; //
	somo_test=*qwd_byt;


    if(cinterfacesmartcard.reciver_buffer[0] == cinterfacesmartcard.transmit_buffer[1])
    {
    	printf("Loading comand \"test_result\"... \n");

    	if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
    	{
    		printf("End comand \"test_result\" : Good = 90 00 \n");
    	}
    }
    else
    {
    	if(cinterfacesmartcard.reciver_buffer[0] == 0x6D)
    	{
    		if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
    		{
    			printf("Error comand \"test_result\" : 6D 00 (Неизвестная команда) \n");
    		}
    	}


    }



	if(cinterfacesmartcard.reciver_buffer[0] == 0x90)
	{
		if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
		{
			printf("End comand \"test_result\" : Good = 90 00 \n");
		}
	}
	else
	{
		if(cinterfacesmartcard.reciver_buffer[0] == 0x63)
								{
									printf("Ошибка аутентификации или предъявления пароля: %d - число оставшихся попыток \n",cinterfacesmartcard.reciver_buffer[1]);
								}

								if(cinterfacesmartcard.reciver_buffer[0] == 0x69)
								{
									switch (cinterfacesmartcard.reciver_buffer[1])
									{
										case 0x82:
												{
													printf("Доступ не разрешен или ошибка предъявления транспортного кода \n : 69 82");
													break;
												}

										case 0x83:
												{
													printf("Ключ, пароль или транспортный код блокирован \n : 69 83");
													break;
												}
										case 0x85:
												{
													printf("Не выполнены условия использования команды (не выполнялись команды GET_CHALLENGE, GEN_SESSION_KEY, нет данных для GET_RESPONSE) \n : 69 85");
													break;
												}
										case 0x86:
												{
													printf("Команда не применима к данному типу файла \n : 69 86");
													break;
												}
										case 0xF0:
												{
													printf("Операция запрещена для данного ключа \n : 69 F0");
													break;
												}
										case 0xF2:
												{
													printf("В данной сессии не выработан диверсифицированный ключ \n : 69 F2");
													break;
												}

									}
								}

								if(cinterfacesmartcard.reciver_buffer[0] == 0x6A)
								{
									switch (cinterfacesmartcard.reciver_buffer[1])
									{
										case 0x83:
										{
											printf("Файл пуст \n : 6A 83");
											break;
										}
										case 0x86:
										{
											printf("Не верные параметры Р1 Р2 \n : 6A 86");
											break;
										}
										case 0x84:
										{
											printf("Не достаточно места для создания файла \n : 6A 84");
											break;
										}
										case 0x80:
										{
											printf("Неприемлемое значение поля данных (файл с указанным идентификатором уже присутствует) \n : 6A 83");
											break;
										}
										case 0x82:
										{
											printf("Файл не найден \n : 6A 82");
											break;
										}
									}
								}
	}
	printf("=========================\n");


	return somo_test;
}


unsigned test_fs()// функция запроса теста системы
{
	unsigned somo_test;
	Ccomanda ccomanda;
	char byt[4];
	unsigned *qwd_byt;
	qwd_byt=(unsigned*)(byt);
	cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
	cinterfacesmartcard.transmit_buffer[1]=0xF0; //передаем номер инстртукции
	cinterfacesmartcard.transmit_buffer[2]=0x00; //ар
	cinterfacesmartcard.transmit_buffer[3]=0x00; //гу
	cinterfacesmartcard.transmit_buffer[4]=0x04;

	write_byte(&ccomanda,5,&cinterfacesmartcard,0);
	while(cinterfacesmartcard.ok==0){}
	cinterfacesmartcard.ok=0;

	byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
	byt[1]=cinterfacesmartcard.reciver_buffer[2]; //
	byt[2]=cinterfacesmartcard.reciver_buffer[3]; //
	somo_test=*qwd_byt;

	int i =0;
	  for(i = 0; i < 1024;i++)
	    {
	    	cinterfacesmartcard.reciver_buffer[i] = 0;
	    }

	return somo_test;
}


unsigned activate(char tk_1, char tk_2)
{
	int somo_test,i;
				Ccomanda ccomanda;
				char byt[4];
				unsigned *qwd_byt;
				qwd_byt=(unsigned*)(byt);

				cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
					cinterfacesmartcard.transmit_buffer[1]=0x20; //передаем номер инстртукции
					cinterfacesmartcard.transmit_buffer[2]=0x00; //ар
					cinterfacesmartcard.transmit_buffer[3]=0x00; //гу
					cinterfacesmartcard.transmit_buffer[4]=0x00; //менты

						write_byte(&ccomanda,5,&cinterfacesmartcard,0);
						while(cinterfacesmartcard.ok==0){}
						cinterfacesmartcard.ok=0;


						byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
						byt[1]=cinterfacesmartcard.reciver_buffer[2]; //
						byt[2]=cinterfacesmartcard.reciver_buffer[3]; //
						byt[3]=0; //
						somo_test=*qwd_byt;
						return somo_test;
}

unsigned delete(char tk_1, char tk_2)
{
	int somo_test,i;
			Ccomanda ccomanda;
			char byt[4];
			unsigned *qwd_byt;
			qwd_byt=(unsigned*)(byt);

			cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
					cinterfacesmartcard.transmit_buffer[1]=0xE4; //передаем номер инстртукции
					cinterfacesmartcard.transmit_buffer[2]=0x00; //ар
					cinterfacesmartcard.transmit_buffer[3]=0x00;
					cinterfacesmartcard.transmit_buffer[4]=0x00;

					write_byte(&ccomanda,5,&cinterfacesmartcard,0);
					while(cinterfacesmartcard.ok==0){}
					cinterfacesmartcard.ok=0;


					 if(cinterfacesmartcard.reciver_buffer[0] == cinterfacesmartcard.transmit_buffer[1])
					 {
					    printf("Loading comand \"delete\"... \n");
					 }
					 else
					 {
					    	if(cinterfacesmartcard.reciver_buffer[0] == 0x6D)
					    	{
					    		if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
					    		{
					    			printf("Error comand \"delete\" : 6D 00 (Неизвестная команда) \n");
					    		}
					    	}
					 }

					cinterfacesmartcard.transmit_buffer[0]=tk_1; //3F
					cinterfacesmartcard.transmit_buffer[1]=tk_2; // 00
					write_byte(&ccomanda,2,&cinterfacesmartcard,0);
					while(cinterfacesmartcard.ok==0){}
					cinterfacesmartcard.ok=0;

					byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
					byt[1]=cinterfacesmartcard.reciver_buffer[2]; //
					byt[2]=cinterfacesmartcard.reciver_buffer[3]; //
					byt[3]=0; //
					somo_test=*qwd_byt;
					return somo_test;

}
//работает
unsigned select(char tk_1, char tk_2,int numer_sim)
{
	printf("=========================\n");
	printf("Start \"select\": \n");
	printf("Go id: 0x%02hhx 0x%02hhx \n",tk_1,tk_2);

	int somo_test,i;
		Ccomanda ccomanda;
		char byt[4];
		unsigned *qwd_byt;
		qwd_byt=(unsigned*)(byt);


		cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
		cinterfacesmartcard.transmit_buffer[1]=0xa4; //передаем номер инстртукции
		cinterfacesmartcard.transmit_buffer[2]=0x21; //ар
		cinterfacesmartcard.transmit_buffer[3]=0x00;

		write_byte(&ccomanda,4,&cinterfacesmartcard,numer_sim);
		while(cinterfacesmartcard.ok==0){}
		cinterfacesmartcard.ok=0;

		cinterfacesmartcard.transmit_buffer[0]=0x02;
		write_byte(&ccomanda,1,&cinterfacesmartcard,numer_sim);
		while(cinterfacesmartcard.ok==0){}
		cinterfacesmartcard.ok=0;


		if(cinterfacesmartcard.reciver_buffer[0] == cinterfacesmartcard.transmit_buffer[1])
		{
		   printf("Loading comand \"select\"... \n");
		}
		else
		{
			if(cinterfacesmartcard.reciver_buffer[0] == 0x6D)
			{
			   if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
			   {
			    	printf("Error comand \"select\" : 6D 00 (Неизвестная команда) \n");
			    }
			}
		}

		cinterfacesmartcard.transmit_buffer[0]=tk_1; //3F
		cinterfacesmartcard.transmit_buffer[1]=tk_2; // 00
		write_byte(&ccomanda,2,&cinterfacesmartcard,numer_sim);
		while(cinterfacesmartcard.ok==0){}
		cinterfacesmartcard.ok=0;

		if(cinterfacesmartcard.reciver_buffer[0] == 0x90)
		{
			if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
			{
				printf("End comand \"select\" : Good = 90 00 \n");
			}
		}
		else
		{
			if(cinterfacesmartcard.reciver_buffer[0] == 0x63)
									{
										printf("Ошибка аутентификации или предъявления пароля: %d - число оставшихся попыток \n",cinterfacesmartcard.reciver_buffer[1]);
									}

									if(cinterfacesmartcard.reciver_buffer[0] == 0x69)
									{
										switch (cinterfacesmartcard.reciver_buffer[1])
										{
											case 0x82:
													{
														printf("Доступ не разрешен или ошибка предъявления транспортного кода \n : 69 82");
														break;
													}

											case 0x83:
													{
														printf("Ключ, пароль или транспортный код блокирован \n : 69 83");
														break;
													}
											case 0x85:
													{
														printf("Не выполнены условия использования команды (не выполнялись команды GET_CHALLENGE, GEN_SESSION_KEY, нет данных для GET_RESPONSE) \n : 69 85");
														break;
													}
											case 0x86:
													{
														printf("Команда не применима к данному типу файла \n : 69 86");
														break;
													}
											case 0xF0:
													{
														printf("Операция запрещена для данного ключа \n : 69 F0");
														break;
													}
											case 0xF2:
													{
														printf("В данной сессии не выработан диверсифицированный ключ \n : 69 F2");
														break;
													}

										}
									}

									if(cinterfacesmartcard.reciver_buffer[0] == 0x6A)
									{
										switch (cinterfacesmartcard.reciver_buffer[1])
										{
											case 0x83:
											{
												printf("Файл пуст \n : 6A 83");
												break;
											}
											case 0x86:
											{
												printf("Не верные параметры Р1 Р2 \n : 6A 86");
												break;
											}
											case 0x84:
											{
												printf("Не достаточно места для создания файла \n : 6A 84");
												break;
											}
											case 0x80:
											{
												printf("Неприемлемое значение поля данных (файл с указанным идентификатором уже присутствует) \n : 6A 83");
												break;
											}
											case 0x82:
											{
												printf("Файл не найден \n : 6A 82");
												break;
											}
										}
									}
		}
		printf("=========================\n");



		byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
		byt[1]=cinterfacesmartcard.reciver_buffer[2]; //
		byt[2]=cinterfacesmartcard.reciver_buffer[3]; //
		byt[3]=0; //
		somo_test=*qwd_byt;
		return somo_test;
}

unsigned update_key(char* key,int number,int numer_sim)
{
		printf("=========================\n");
		printf("Start \"update_key\": \n");

		int somo_test;
		Ccomanda ccomanda;
		char byt[4];
		unsigned *qwd_byt;
		qwd_byt=(unsigned*)(byt);
		cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
		cinterfacesmartcard.transmit_buffer[1]=0xDC; //передаем номер инстртукции
		cinterfacesmartcard.transmit_buffer[2]=0x00; //ар
		cinterfacesmartcard.transmit_buffer[3]=0x00; //


		int i =0;
		switch(number)
		{
		case 0:
				{
					printf("key (для пароля): \n");
					for(i=0; i < 8;i++)
					{
						printf("%d) 0x%02hhx\n",i+1,key[i]);
					}
					cinterfacesmartcard.transmit_buffer[4]=0x08;
					break;
				}
		case 1:
				{
					printf("key (для ключа без признака спецмер): \n");
					for(i=0; i < 32;i++)
					{
						printf("%d) 0x%02hhx\n",i+1,key[i]);
					}
					cinterfacesmartcard.transmit_buffer[4]=0x20;
					break;
				}
		case 2:
				{
					printf("key (для ключа с признаком спецмер): \n");
					for(i=0; i < 36;i++)
					{
						printf("%d) 0x%02hhx\n",i+1,key[i]);
					}
					cinterfacesmartcard.transmit_buffer[4]=0x24;
					break;
				}
		}


				write_byte(&ccomanda,5,&cinterfacesmartcard,numer_sim);
				while(cinterfacesmartcard.ok==0){}
				cinterfacesmartcard.ok=0;

				if(cinterfacesmartcard.reciver_buffer[0] == cinterfacesmartcard.transmit_buffer[1])
					{
						printf("Loading comand \"update_key\"... \n");
					}
					else
					{
						if(cinterfacesmartcard.reciver_buffer[0] == 0x6D)
						{
							if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
							{
								printf("Error comand \"update_key\" : 6D 00 (Неизвестная команда) \n");
							}
						}
					}


				switch(number)
				{
					case 0:
						{
							cinterfacesmartcard.transmit_buffer[0]= key[0] ;
							cinterfacesmartcard.transmit_buffer[1]= key[1] ;
							cinterfacesmartcard.transmit_buffer[2]= key[2] ;
							cinterfacesmartcard.transmit_buffer[3]= key[3] ;
							cinterfacesmartcard.transmit_buffer[4]= key[4] ;
							cinterfacesmartcard.transmit_buffer[5]= key[5] ;
							cinterfacesmartcard.transmit_buffer[6]= key[6] ;
							cinterfacesmartcard.transmit_buffer[7]= key[7] ;

							write_byte(&ccomanda,8,&cinterfacesmartcard,numer_sim);
							while(cinterfacesmartcard.ok==0){}
							cinterfacesmartcard.ok=0;

							break;
						}
					case 1:
						{

							int i=0;

							for( i =0; i < 32;i++)
							{
								cinterfacesmartcard.transmit_buffer[i]= key[i] ;
							}

							write_byte(&ccomanda,32,&cinterfacesmartcard,numer_sim);
							while(cinterfacesmartcard.ok==0){}
							cinterfacesmartcard.ok=0;

							break;

						}
					case 2:
						{
							int i=0;

							for( i =0; i < 36;i++)
							{
								cinterfacesmartcard.transmit_buffer[i]= key[i] ;
							}

							write_byte(&ccomanda,36,&cinterfacesmartcard,numer_sim);
							while(cinterfacesmartcard.ok==0){}
							cinterfacesmartcard.ok=0;

							break;
						}
				}

				if(cinterfacesmartcard.reciver_buffer[0] == 0x90)
				{
					if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
					{
						printf("End comand \"update_key\" : Good = 90 00 \n");
					}
				}
				else
				{
					if(cinterfacesmartcard.reciver_buffer[0] == 0x63)
					{
						printf("Ошибка аутентификации или предъявления пароля: %d - число оставшихся попыток \n",cinterfacesmartcard.reciver_buffer[1]);
					}

					if(cinterfacesmartcard.reciver_buffer[0] == 0x69)
					{
						switch (cinterfacesmartcard.reciver_buffer[1])
						{
							case 0x82:
									{
										printf("Доступ не разрешен или ошибка предъявления транспортного кода \n : 69 82");
										break;
									}

							case 0x83:
									{
										printf("Ключ, пароль или транспортный код блокирован \n : 69 83");
										break;
									}
							case 0x85:
									{
										printf("Не выполнены условия использования команды (не выполнялись команды GET_CHALLENGE, GEN_SESSION_KEY, нет данных для GET_RESPONSE) \n : 69 85");
										break;
									}
							case 0x86:
									{
										printf("Команда не применима к данному типу файла \n : 69 86");
										break;
									}
							case 0xF0:
									{
										printf("Операция запрещена для данного ключа \n : 69 F0");
										break;
									}
							case 0xF2:
									{
										printf("В данной сессии не выработан диверсифицированный ключ \n : 69 F2");
										break;
									}

						}
					}

					if(cinterfacesmartcard.reciver_buffer[0] == 0x6A)
					{
						switch (cinterfacesmartcard.reciver_buffer[1])
						{
							case 0x83:
							{
								printf("Файл пуст \n : 6A 83");
								break;
							}
							case 0x86:
							{
								printf("Не верные параметры Р1 Р2 \n : 6A 86");
								break;
							}
							case 0x84:
							{
								printf("Не достаточно места для создания файла \n : 6A 84");
								break;
							}
						}
					}

				}

				printf("=========================\n");


				byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
				byt[1]=cinterfacesmartcard.reciver_buffer[2]; //
				byt[2]=cinterfacesmartcard.reciver_buffer[3]; //
				byt[3]=0; //
				somo_test=*qwd_byt;
				return somo_test;
}

unsigned load_key(char* key,int number, int numer_sim)
{
	printf("=========================\n");
	printf("Start \"load_key\": \n");

	int somo_test;
	Ccomanda ccomanda;
	char byt[4];
	unsigned *qwd_byt;
	qwd_byt=(unsigned*)(byt);
	cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
	cinterfacesmartcard.transmit_buffer[1]=0xE2; //передаем номер инстртукции
	cinterfacesmartcard.transmit_buffer[2]=0x00; //ар
	cinterfacesmartcard.transmit_buffer[3]=0x00; //


	int i =0;
	switch(number)
	{
	case 0:
			{
				printf("key (для пароля): \n");
				for(i=0; i < 8;i++)
				{
					printf("%d) 0x%02hhx\n",i+1,key[i]);
				}
				cinterfacesmartcard.transmit_buffer[4]=0x08;
				break;
			}
	case 1:
			{
				printf("key (для ключа без признака спецмер): \n");
				for(i=0; i < 32;i++)
				{
					printf("%d) 0x%02hhx\n",i+1,key[i]);
				}
				cinterfacesmartcard.transmit_buffer[4]=0x20;
				break;
			}
	case 2:
			{
				printf("key (для ключа с признаком спецмер): \n");
				for(i=0; i < 36;i++)
				{
					printf("%d) 0x%02hhx\n",i+1,key[i]);
				}
				cinterfacesmartcard.transmit_buffer[4]=0x24;
				break;
			}
	}


			write_byte(&ccomanda,5,&cinterfacesmartcard,numer_sim);
			while(cinterfacesmartcard.ok==0){}
			cinterfacesmartcard.ok=0;

			if(cinterfacesmartcard.reciver_buffer[0] == cinterfacesmartcard.transmit_buffer[1])
				{
					printf("Loading comand \"load_key\"... \n");
				}
				else
				{
					if(cinterfacesmartcard.reciver_buffer[0] == 0x6D)
					{
						if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
						{
							printf("Error comand \"load_key\" : 6D 00 (Неизвестная команда) \n");
						}
					}
				}


			switch(number)
			{
				case 0:
					{
						cinterfacesmartcard.transmit_buffer[0]= key[0] ;
						cinterfacesmartcard.transmit_buffer[1]= key[1] ;
						cinterfacesmartcard.transmit_buffer[2]= key[2] ;
						cinterfacesmartcard.transmit_buffer[3]= key[3] ;
						cinterfacesmartcard.transmit_buffer[4]= key[4] ;
						cinterfacesmartcard.transmit_buffer[5]= key[5] ;
						cinterfacesmartcard.transmit_buffer[6]= key[6] ;
						cinterfacesmartcard.transmit_buffer[7]= key[7] ;

						write_byte(&ccomanda,8,&cinterfacesmartcard,numer_sim);
						while(cinterfacesmartcard.ok==0){}
						cinterfacesmartcard.ok=0;

						break;
					}
				case 1:
					{

						int i=0;

						for( i =0; i < 32;i++)
						{
							cinterfacesmartcard.transmit_buffer[i]= key[i] ;
						}

						write_byte(&ccomanda,32,&cinterfacesmartcard,numer_sim);
						while(cinterfacesmartcard.ok==0){}
						cinterfacesmartcard.ok=0;

						break;

					}
				case 2:
					{
						int i=0;

						for( i =0; i < 36;i++)
						{
							cinterfacesmartcard.transmit_buffer[i]= key[i] ;
						}

						write_byte(&ccomanda,36,&cinterfacesmartcard,numer_sim);
						while(cinterfacesmartcard.ok==0){}
						cinterfacesmartcard.ok=0;

						break;
					}
			}

			if(cinterfacesmartcard.reciver_buffer[0] == 0x90)
			{
				if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
				{
					printf("End comand \"load_key\" : Good = 90 00 \n");
				}
			}
			else
			{
				if(cinterfacesmartcard.reciver_buffer[0] == 0x63)
				{
					printf("Ошибка аутентификации или предъявления пароля: %d - число оставшихся попыток \n",cinterfacesmartcard.reciver_buffer[1]);
				}

				if(cinterfacesmartcard.reciver_buffer[0] == 0x69)
				{
					switch (cinterfacesmartcard.reciver_buffer[1])
					{
						case 0x82:
								{
									printf("Доступ не разрешен или ошибка предъявления транспортного кода \n : 69 82");
									break;
								}

						case 0x83:
								{
									printf("Ключ, пароль или транспортный код блокирован \n : 69 83");
									break;
								}
						case 0x85:
								{
									printf("Не выполнены условия использования команды (не выполнялись команды GET_CHALLENGE, GEN_SESSION_KEY, нет данных для GET_RESPONSE) \n : 69 85");
									break;
								}
						case 0x86:
								{
									printf("Команда не применима к данному типу файла \n : 69 86");
									break;
								}
						case 0xF0:
								{
									printf("Операция запрещена для данного ключа \n : 69 F0");
									break;
								}
						case 0xF2:
								{
									printf("В данной сессии не выработан диверсифицированный ключ \n : 69 F2");
									break;
								}

					}
				}

				if(cinterfacesmartcard.reciver_buffer[0] == 0x6A)
				{
					switch (cinterfacesmartcard.reciver_buffer[1])
					{
						case 0x83:
						{
							printf("Файл пуст \n : 6A 83");
							break;
						}
						case 0x86:
						{
							printf("Не верные параметры Р1 Р2 \n : 6A 86");
							break;
						}
						case 0x84:
						{
							printf("Не достаточно места для создания файла \n : 6A 84");
							break;
						}
					}
				}

			}

			printf("=========================\n");


			byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
			byt[1]=cinterfacesmartcard.reciver_buffer[2]; //
			byt[2]=cinterfacesmartcard.reciver_buffer[3]; //
			byt[3]=0; //
			somo_test=*qwd_byt;
			return somo_test;
}

char* get_session_key(char* mass, int numer_sim)
{
	int somo_test;
			Ccomanda ccomanda;
			char byt[6];

			unsigned *qwd_byt;
			qwd_byt=(unsigned*)(byt);
			cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
			cinterfacesmartcard.transmit_buffer[1]=0x4a; //передаем номер инстртукции
			cinterfacesmartcard.transmit_buffer[2]=0x00; //ар
			cinterfacesmartcard.transmit_buffer[3]=0x00; //
			cinterfacesmartcard.transmit_buffer[4]=0x10;

			write_byte(&ccomanda,5,&cinterfacesmartcard,numer_sim);
						while(cinterfacesmartcard.ok==0){}
						cinterfacesmartcard.ok=0;


			int i;

			for( i =0 ; i < 16; i++)
			{
				cinterfacesmartcard.transmit_buffer[i] = mass[i];
			}

			write_byte(&ccomanda,16,&cinterfacesmartcard,numer_sim);
			while(cinterfacesmartcard.ok==0){}
			cinterfacesmartcard.ok=0;

			cinterfacesmartcard.transmit_buffer[0] = 0x06;

			write_byte(&ccomanda,1,&cinterfacesmartcard,numer_sim);
			while(cinterfacesmartcard.ok==0){}
			cinterfacesmartcard.ok=0;

			/*
			cinterfacesmartcard.transmit_buffer[0] = 0x4A;


			write_byte(&ccomanda,1,&cinterfacesmartcard,0);
			while(cinterfacesmartcard.ok==0){}
			cinterfacesmartcard.ok=0;

			cinterfacesmartcard.transmit_buffer[0] = 0x00;


			write_byte(&ccomanda,1,&cinterfacesmartcard,0);
			while(cinterfacesmartcard.ok==0){}
			cinterfacesmartcard.ok=0;

			cinterfacesmartcard.transmit_buffer[0] = 0x00;


			write_byte(&ccomanda,1,&cinterfacesmartcard,0);
			while(cinterfacesmartcard.ok==0){}
			cinterfacesmartcard.ok=0;

			cinterfacesmartcard.transmit_buffer[0] = 0x10;


			write_byte(&ccomanda,1,&cinterfacesmartcard,0);
			while(cinterfacesmartcard.ok==0){}
			cinterfacesmartcard.ok=0;

			*/


			byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
			byt[1]=cinterfacesmartcard.reciver_buffer[2]; //
			byt[2]=cinterfacesmartcard.reciver_buffer[3]; //
			byt[3]=cinterfacesmartcard.reciver_buffer[4]; //
			byt[4]=cinterfacesmartcard.reciver_buffer[5]; //
			byt[5]=cinterfacesmartcard.reciver_buffer[6]; //
			somo_test=*qwd_byt;



			return *byt;
}


//работает
char* get_challenge(char L,int numer_sim)
{
	printf("=========================\n");
	printf("Start \"get_challenge\": \n");

	int somo_test;
		Ccomanda ccomanda;
		char* byt2;

		unsigned *qwd_byt;

		cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
		cinterfacesmartcard.transmit_buffer[1]=0x84; //передаем номер инстртукции
		cinterfacesmartcard.transmit_buffer[2]=0x00; //ар
		cinterfacesmartcard.transmit_buffer[3]=0x00; //
		cinterfacesmartcard.transmit_buffer[4]=L; // 0x08
		write_byte(&ccomanda,5,&cinterfacesmartcard,numer_sim);
		while(cinterfacesmartcard.ok==0){}
		cinterfacesmartcard.ok=0;



		switch (L) {
			case 0x08:{
							if(cinterfacesmartcard.reciver_buffer[9] == 0x90 )
										{
											if(cinterfacesmartcard.reciver_buffer[10] == 0x00)
											{
												printf("End comand \"get_challenge\" : Good = 90 00 \n");
											}
										}
									printf("=========================\n");


									byt2[0]=cinterfacesmartcard.reciver_buffer[1];
									byt2[1]=cinterfacesmartcard.reciver_buffer[2];
									byt2[2]=cinterfacesmartcard.reciver_buffer[3];
									byt2[3]=cinterfacesmartcard.reciver_buffer[4];
									byt2[4]=cinterfacesmartcard.reciver_buffer[5];
									byt2[5]=cinterfacesmartcard.reciver_buffer[6];
									byt2[6]=cinterfacesmartcard.reciver_buffer[7];
									byt2[7]=cinterfacesmartcard.reciver_buffer[8];

						return byt2;
						break;
			}
			case 0x10:
			{

									if(cinterfacesmartcard.reciver_buffer[17] == 0x90)
														{
															if(cinterfacesmartcard.reciver_buffer[18] == 0x00)
															{
																printf("End comand \"get_challenge\" : Good = 90 00 \n");
															}
														}
													printf("=========================\n");


									byt2[0]=cinterfacesmartcard.reciver_buffer[1];
									byt2[1]=cinterfacesmartcard.reciver_buffer[2];
									byt2[2]=cinterfacesmartcard.reciver_buffer[3];
									byt2[3]=cinterfacesmartcard.reciver_buffer[4];
									byt2[4]=cinterfacesmartcard.reciver_buffer[5];
									byt2[5]=cinterfacesmartcard.reciver_buffer[6];
									byt2[6]=cinterfacesmartcard.reciver_buffer[7];
									byt2[7]=cinterfacesmartcard.reciver_buffer[8];
									byt2[8]=cinterfacesmartcard.reciver_buffer[9];
									byt2[9]=cinterfacesmartcard.reciver_buffer[10];
									byt2[10]=cinterfacesmartcard.reciver_buffer[11];
									byt2[11]=cinterfacesmartcard.reciver_buffer[12];
									byt2[12]=cinterfacesmartcard.reciver_buffer[13];
									byt2[13]=cinterfacesmartcard.reciver_buffer[14];
									byt2[14]=cinterfacesmartcard.reciver_buffer[15];
									byt2[15]=cinterfacesmartcard.reciver_buffer[16];

						return byt2;
						break;
			}

		}

		return 0;

}


unsigned unlock_key(int numer_sim)
{
	int somo_test;
			Ccomanda ccomanda;
			char byt[4];
			unsigned *qwd_byt;
			qwd_byt=(unsigned*)(byt);
			cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
			cinterfacesmartcard.transmit_buffer[1]=0x4e; //передаем номер инстртукции
			cinterfacesmartcard.transmit_buffer[2]=0x00; //ар
			cinterfacesmartcard.transmit_buffer[3]=0x00; //
			cinterfacesmartcard.transmit_buffer[4]=0x00; //
			write_byte(&ccomanda,5,&cinterfacesmartcard,numer_sim);
			while(cinterfacesmartcard.ok==0){}
			cinterfacesmartcard.ok=0;
			byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
			byt[1]=cinterfacesmartcard.reciver_buffer[2]; //
			byt[2]=cinterfacesmartcard.reciver_buffer[3]; //
			byt[3]=0; //
			somo_test=*qwd_byt;
			return somo_test;
}

char check_rom(char* key,int numer_sim)
{
	int somo_test;
		Ccomanda ccomanda;

		char byt;

		cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
		cinterfacesmartcard.transmit_buffer[1]=0x52; //передаем номер инстртукции
		cinterfacesmartcard.transmit_buffer[2]=0x00; //ар
		cinterfacesmartcard.transmit_buffer[3]=0x00;
		cinterfacesmartcard.transmit_buffer[4]=0x20;

		write_byte(&ccomanda,5,&cinterfacesmartcard,numer_sim);
		while(cinterfacesmartcard.ok==0){}
		cinterfacesmartcard.ok=0;

		cinterfacesmartcard.transmit_buffer[0] = 0x04;

		int i=0;
		for(i=1; i < 33;i++)
		{
			cinterfacesmartcard.transmit_buffer[i] = key[i-1];
		}

		write_byte(&ccomanda,33,&cinterfacesmartcard,numer_sim);
				while(cinterfacesmartcard.ok==0){}
				cinterfacesmartcard.ok=0;


		byt = cinterfacesmartcard.reciver_buffer[257]; //


		return byt;
}


unsigned write_tk_pl(int numer_sim)
{

	int somo_test;
	Ccomanda ccomanda;
	char byt[4];
	unsigned *qwd_byt;
	qwd_byt=(unsigned*)(byt);
	cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
	cinterfacesmartcard.transmit_buffer[1]=0x3A; //передаем номер инстртукции
	cinterfacesmartcard.transmit_buffer[2]=0x00; //ар
	cinterfacesmartcard.transmit_buffer[3]=0x00; //гу
	cinterfacesmartcard.transmit_buffer[4]=0x08; //менты


	write_byte(&ccomanda,5,&cinterfacesmartcard,numer_sim);
		while(cinterfacesmartcard.ok==0){}
		cinterfacesmartcard.ok=0;


	cinterfacesmartcard.transmit_buffer[0]=0x20;//0xAB;
	cinterfacesmartcard.transmit_buffer[1]=0x20;//0x49;
	cinterfacesmartcard.transmit_buffer[2]=0x20;//0xB8;
	cinterfacesmartcard.transmit_buffer[3]=0x20;//0xF7;
	cinterfacesmartcard.transmit_buffer[4]=0x20;//0xAB;
	cinterfacesmartcard.transmit_buffer[5]=0x20;//0x49;
	cinterfacesmartcard.transmit_buffer[6]=0x20;//0xB8;
	cinterfacesmartcard.transmit_buffer[7]=0x20;//0xF7;
	cinterfacesmartcard.transmit_buffer[8]=0x20;
	write_byte(&ccomanda,9,&cinterfacesmartcard,numer_sim);
	while(cinterfacesmartcard.ok==0){}
	cinterfacesmartcard.ok=0;

	byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
	byt[1]=cinterfacesmartcard.reciver_buffer[2]; //
	byt[2]=cinterfacesmartcard.reciver_buffer[3]; //
	byt[3]=0; //
	somo_test=*qwd_byt;

	int i =0;

	    for(i = 0; i < 1024;i++)
	    {
	    	cinterfacesmartcard.transmit_buffer[i] = 0;
	    }



	return somo_test;
}


unsigned update_binary(char* data,char size_ef,int kod, int numer_sim)
{
	printf("=========================\n");
	printf("Start \"update_binary\": \n");
	printf("Size: 0x%02hhx \n", size_ef);
	printf("Data: \n");
	int i=0;

	for(i = 0; i < (int)size_ef;i++)
	{
		printf("0x%02hhx\n",data[i]);
	}

	int somo_test;
			Ccomanda ccomanda;
			char byt[4];
			unsigned *qwd_byt;
			qwd_byt=(unsigned*)(byt);
			cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
			cinterfacesmartcard.transmit_buffer[1]=0xD6; //передаем номер инстртукции
			cinterfacesmartcard.transmit_buffer[2]=0x00; //ар
			cinterfacesmartcard.transmit_buffer[3]=0x00; //гу

			switch (kod) {
					case 0:
					{
						printf("Запись данных не шифрованна \n");
						cinterfacesmartcard.transmit_buffer[4]=size_ef;
						break;
					}


					case 1:
					{
						printf("Запись данные зашифрованна \n");
						cinterfacesmartcard.transmit_buffer[4]=size_ef+0x0c;
						break;
					}

				}



			write_byte(&ccomanda,5,&cinterfacesmartcard,numer_sim);
			while(cinterfacesmartcard.ok==0){}
			cinterfacesmartcard.ok=0;


			if(cinterfacesmartcard.reciver_buffer[0] == cinterfacesmartcard.transmit_buffer[1])
			{
				printf("Loading comand \"update_binary\"... \n");
			}
			else
			{
				if(cinterfacesmartcard.reciver_buffer[0] == 0x6D)
				{
					if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
					{
						printf("Error comand \"update_binary\" : 6D 00 (Неизвестная команда) \n");
					}
				}
			}


			i=0;
			if(kod == 0)
			{

				for(i = 0; i < (int)size_ef;i++)
				{
					cinterfacesmartcard.transmit_buffer[i] = data[i];
				}

				write_byte(&ccomanda,(int)size_ef,&cinterfacesmartcard,numer_sim);
				while(cinterfacesmartcard.ok==0){}
				cinterfacesmartcard.ok=0;
			}
			else
			{

				for(i = 0; i < (int)(size_ef+0x0c);i++)
				{
					cinterfacesmartcard.transmit_buffer[i] = data[i];
				}

				write_byte(&ccomanda,(int)(size_ef+0x0c),&cinterfacesmartcard,numer_sim);
				while(cinterfacesmartcard.ok==0){}
				cinterfacesmartcard.ok=0;
			}


			if(cinterfacesmartcard.reciver_buffer[0] == 0x90)
			{
				if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
				{
					printf("End comand \"update_binary\" : Good = 90 00 \n");
				}
			}
			printf("=========================\n");


			byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
			byt[1]=cinterfacesmartcard.reciver_buffer[2]; //
			byt[2]=cinterfacesmartcard.reciver_buffer[3]; //
			byt[3]=0; //
			somo_test=*qwd_byt;
			return somo_test;
}

char* read_binary(char size_ef , int kod , int numer_sim)
{
	printf("=========================\n");
	printf("Start \"read_binary\": \n");

	int somo_test;
		Ccomanda ccomanda;
		char* byt;
		unsigned *qwd_byt;
		qwd_byt=(unsigned*)(byt);
		cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
		cinterfacesmartcard.transmit_buffer[1]=0xB0; //передаем номер инстртукции
		cinterfacesmartcard.transmit_buffer[2]=0x00; //ар
		cinterfacesmartcard.transmit_buffer[3]=0x00; //гу


		switch (kod) {
			case 0:
			{
				printf("Чтение данных не шифрованна \n");
				cinterfacesmartcard.transmit_buffer[4]=size_ef;
				break;
			}


			case 1:
			{
				printf("Чтение данных зашифрованна \n");
				cinterfacesmartcard.transmit_buffer[4]=size_ef+0x0c;
				break;
			}

		}


		write_byte(&ccomanda,5,&cinterfacesmartcard,numer_sim);
		while(cinterfacesmartcard.ok==0){}
		cinterfacesmartcard.ok=0;


		if(cinterfacesmartcard.reciver_buffer[0] == cinterfacesmartcard.transmit_buffer[1])
		{
			printf("Loading comand \"read_binary\"... \n");

			int i=1;

			while((cinterfacesmartcard.reciver_buffer[i] == 0x90) && (cinterfacesmartcard.reciver_buffer[i] == 0x00))
			{

				printf("0x%02hhx\n",cinterfacesmartcard.reciver_buffer[i]);
				i++;

			}

			printf("End comand \"select\" : Good = 90 00 \n");


		}
		else
		{
			if(cinterfacesmartcard.reciver_buffer[0] == 0x6D)
			{
				if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
				{
					printf("Error comand \"read_binary\" : 6D 00 (Неизвестная команда) \n");
				}
			}
		}

		int i=0;

		for(i=0 ; i <  (int)size_ef;i++)
		{
			byt[i]=cinterfacesmartcard.reciver_buffer[i+1];
		}

		return byt;
}


//работает если Мастер файла не существует (обратиться к нему можно командой SELECT по адресу 3F 00
unsigned create_mf(int numer_sim)
{
	printf("=========================\n");
	printf("Start \"create_mf\": \n");
	printf("S =  00  \n");
	printf("AC =  00 00 00 00 00 00 00 00 \n");
	int somo_test;
	Ccomanda ccomanda;
	char byt[4];
	unsigned *qwd_byt;
	qwd_byt=(unsigned*)(byt);
	cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
	cinterfacesmartcard.transmit_buffer[1]=0x30; //передаем номер инстртукции
	cinterfacesmartcard.transmit_buffer[2]=0x00; //ар
	cinterfacesmartcard.transmit_buffer[3]=0x00; //гу
	cinterfacesmartcard.transmit_buffer[4]=0x09;

	write_byte(&ccomanda,5,&cinterfacesmartcard,numer_sim);
	while(cinterfacesmartcard.ok==0){}
	cinterfacesmartcard.ok=0;

	  if(cinterfacesmartcard.reciver_buffer[0] == cinterfacesmartcard.transmit_buffer[1])
	    {
	    	printf("Loading comand \"create_mf\"... \n");
	    }
	    else
	    {
	    	if(cinterfacesmartcard.reciver_buffer[0] == 0x6D)
	    	{
	    		if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
	    		{
	    			printf("Error comand \"create_mf\" : 6D 00 (Неизвестная команда) \n");
	    		}
	    	}
	    }

	cinterfacesmartcard.transmit_buffer[0]=0x00;

	write_byte(&ccomanda,1,&cinterfacesmartcard,numer_sim);
	while(cinterfacesmartcard.ok==0){}
	cinterfacesmartcard.ok=0;


	cinterfacesmartcard.transmit_buffer[0]=0x00; //менты
	cinterfacesmartcard.transmit_buffer[1]=0x00;
	cinterfacesmartcard.transmit_buffer[2]=0x00;
	cinterfacesmartcard.transmit_buffer[3]=0x00;
	cinterfacesmartcard.transmit_buffer[4]=0x00;
	cinterfacesmartcard.transmit_buffer[5]=0x00;
	cinterfacesmartcard.transmit_buffer[6]=0x00;
	cinterfacesmartcard.transmit_buffer[7]=0x00;

	write_byte(&ccomanda,8,&cinterfacesmartcard,numer_sim);
	while(cinterfacesmartcard.ok==0){}
	cinterfacesmartcard.ok=0;



	if(cinterfacesmartcard.reciver_buffer[0] == 0x90)
	{
		if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
		{
			printf("End comand \"create_mf\" : Good = 90 00 \n");
		}
	}
	printf("=========================\n");


	byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
	byt[1]=cinterfacesmartcard.reciver_buffer[2]; //
	byt[2]=cinterfacesmartcard.reciver_buffer[3]; //
	byt[3]=0; //
	somo_test=*qwd_byt;
	return somo_test;
}

unsigned restore()
{
	int somo_test;
		Ccomanda ccomanda;
		char byt[4];
		unsigned *qwd_byt;
		qwd_byt=(unsigned*)(byt);
		cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
		cinterfacesmartcard.transmit_buffer[1]=0xf4; //передаем номер инстртукции
		cinterfacesmartcard.transmit_buffer[2]=0x00; //ар
		cinterfacesmartcard.transmit_buffer[3]=0x00; //гу
		cinterfacesmartcard.transmit_buffer[4]=0x00;
		write_byte(&ccomanda,5,&cinterfacesmartcard,1);
		while(cinterfacesmartcard.ok==0){}
		cinterfacesmartcard.ok=0;
		byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
		byt[1]=cinterfacesmartcard.reciver_buffer[2]; //
		byt[2]=cinterfacesmartcard.reciver_buffer[3]; //
		byt[3]=0;
		somo_test=*qwd_byt;

		int i =0;
			  for(i = 0; i < 1024;i++)
			    {
			    	cinterfacesmartcard.reciver_buffer[i] = 0;
			    }

		return somo_test;
}
char* get_response(char size, int numer_sim)
{
	printf("=========================\n");
	printf("Start \"get_response\": \n");

	int somo_test;
	Ccomanda ccomanda;
	char* byt;

	cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
	cinterfacesmartcard.transmit_buffer[1]=0xc0; //передаем номер инстртукции
	cinterfacesmartcard.transmit_buffer[2]=0x00; //ар
	cinterfacesmartcard.transmit_buffer[3]=0x00; //гу
	cinterfacesmartcard.transmit_buffer[4]=size; //0xFA; //менты

	write_byte(&ccomanda,5,&cinterfacesmartcard,numer_sim);
	while(cinterfacesmartcard.ok==0){}
	cinterfacesmartcard.ok=0;


	if(cinterfacesmartcard.reciver_buffer[0] == 0x90)
			{
				if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
				{
					printf("End comand \"external_authenticate\" : Good = 90 00 \n");
				}
			}
			else
			{
				if(cinterfacesmartcard.reciver_buffer[0] == 0x63)
										{
											printf("Ошибка аутентификации или предъявления пароля: %d - число оставшихся попыток \n",cinterfacesmartcard.reciver_buffer[1]);
										}

										if(cinterfacesmartcard.reciver_buffer[0] == 0x69)
										{
											switch (cinterfacesmartcard.reciver_buffer[1])
											{
												case 0x82:
														{
															printf("Доступ не разрешен или ошибка предъявления транспортного кода \n : 69 82");
															break;
														}

												case 0x83:
														{
															printf("Ключ, пароль или транспортный код блокирован \n : 69 83");
															break;
														}
												case 0x85:
														{
															printf("Не выполнены условия использования команды (не выполнялись команды GET_CHALLENGE, GEN_SESSION_KEY, нет данных для GET_RESPONSE) \n : 69 85");
															break;
														}
												case 0x86:
														{
															printf("Команда не применима к данному типу файла \n : 69 86");
															break;
														}
												case 0xF0:
														{
															printf("Операция запрещена для данного ключа \n : 69 F0");
															break;
														}
												case 0xF2:
														{
															printf("В данной сессии не выработан диверсифицированный ключ \n : 69 F2");
															break;
														}

											}
										}

										if(cinterfacesmartcard.reciver_buffer[0] == 0x6A)
										{
											switch (cinterfacesmartcard.reciver_buffer[1])
											{
												case 0x83:
												{
													printf("Файл пуст \n : 6A 83");
													break;
												}
												case 0x86:
												{
													printf("Не верные параметры Р1 Р2 \n : 6A 86");
													break;
												}
												case 0x84:
												{
													printf("Не достаточно места для создания файла \n : 6A 84");
													break;
												}
												case 0x80:
												{
													printf("Неприемлемое значение поля данных (файл с указанным идентификатором уже присутствует) \n : 6A 80");
													break;
												}
											}
										}
			}
			printf("=========================\n");

			int i=0;

			for(i=0 ; i <  (int)size;i++)
			{
				byt[i]=cinterfacesmartcard.reciver_buffer[i+1];
			}

	//byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
	//byt[1]=cinterfacesmartcard.reciver_buffer[2]; //
	//byt[2]=cinterfacesmartcard.reciver_buffer[3]; //
	//byt[3]=cinterfacesmartcard.reciver_buffer[4];
	//byt[4]=cinterfacesmartcard.reciver_buffer[5];
	//byt[5]=cinterfacesmartcard.reciver_buffer[6];



	return byt;
}


unsigned write_tk(int numer_sim)
{
	int somo_test;
	Ccomanda ccomanda;
	char byt[4];
	unsigned *qwd_byt;
	qwd_byt=(unsigned*)(byt);
	cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
	cinterfacesmartcard.transmit_buffer[1]=0x36; //передаем номер инстртукции
	cinterfacesmartcard.transmit_buffer[2]=0x00; //ар
	cinterfacesmartcard.transmit_buffer[3]=0x00; //гу
	cinterfacesmartcard.transmit_buffer[4]=0x08; //менты
	//write_byte(&ccomanda,5,&cinterfacesmartcard,0);
	//while(cinterfacesmartcard.ok==0){}
	//cinterfacesmartcard.ok=0;
	//cinterfacesmartcard.transmit_buffer[5]=0xab;//0xAB;
	//cinterfacesmartcard.transmit_buffer[6]=0x49;//0x49;
	//cinterfacesmartcard.transmit_buffer[7]=0xb8;//0xB8;
	//cinterfacesmartcard.transmit_buffer[8]=0xfB;//0xF7;
	//cinterfacesmartcard.transmit_buffer[9]=0xab;//0xAB;
	//cinterfacesmartcard.transmit_buffer[10]=0x49;//0x49;
	//cinterfacesmartcard.transmit_buffer[11]=0xb8;//0xB8;
	//cinterfacesmartcard.transmit_buffer[12]=0xfB;//0xF7;
	//cinterfacesmartcard.transmit_buffer[13]=0x00;

	cinterfacesmartcard.transmit_buffer[5]=0xab;//0xAB;
		cinterfacesmartcard.transmit_buffer[6]=0x49;//0x49;
		cinterfacesmartcard.transmit_buffer[7]=0xb8;//0xB8;
		cinterfacesmartcard.transmit_buffer[8]=0xfb;//0xF7;
		cinterfacesmartcard.transmit_buffer[9]=0xab;//0xAB;
		cinterfacesmartcard.transmit_buffer[10]=0x49;//0x49;
		cinterfacesmartcard.transmit_buffer[11]=0xb8;//0xB8;
		cinterfacesmartcard.transmit_buffer[12]=0xfb;//0xF7;
	//	cinterfacesmartcard.transmit_buffer[13]=0x00;

	write_byte(&ccomanda,13,&cinterfacesmartcard,numer_sim);
	while(cinterfacesmartcard.ok==0){}
	cinterfacesmartcard.ok=0;
	byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
	byt[1]=cinterfacesmartcard.reciver_buffer[2]; //
	byt[2]=cinterfacesmartcard.reciver_buffer[3]; //
	byt[3]=0; //
	somo_test=*qwd_byt;


	int i =0;

	    for(i = 0; i < 1024;i++)
	    {
	    	cinterfacesmartcard.transmit_buffer[i] = 0;
	    }

	//int i =0;
	//for(i = 0; i < 1024;i++)
	//{
	//	cinterfacesmartcard.reciver_buffer[i] = 0;
	//}

	return somo_test;
}

unsigned lock_key(int numer_sim)
{
	int somo_test;
			Ccomanda ccomanda;
			char byt[4];
			unsigned *qwd_byt;
			qwd_byt=(unsigned*)(byt);
			cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
			cinterfacesmartcard.transmit_buffer[1]=0x4C; //передаем номер инстртукции
			cinterfacesmartcard.transmit_buffer[2]=0x00; //ар
			cinterfacesmartcard.transmit_buffer[3]=0x00; //гу
			cinterfacesmartcard.transmit_buffer[4]=0x00; //менты

			write_byte(&ccomanda,5,&cinterfacesmartcard,numer_sim);
			while(cinterfacesmartcard.ok==0){}
		    cinterfacesmartcard.ok=0;
		    byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
		    byt[1]=cinterfacesmartcard.reciver_buffer[2]; //
		    byt[2]=cinterfacesmartcard.reciver_buffer[3]; //
		    byt[3]=0; //
		    somo_test=*qwd_byt;



	return somo_test;

}

unsigned verify(char* key,int number, int numer_sim)
{
	printf("=========================\n");
	printf("Start \"verify\": \n");

	int somo_test;
	Ccomanda ccomanda;
	char byt[4];
	unsigned *qwd_byt;
	qwd_byt=(unsigned*)(byt);
	cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
	cinterfacesmartcard.transmit_buffer[1]=0x20; //передаем номер инстртукции
	cinterfacesmartcard.transmit_buffer[2]=0x00; //ар
	cinterfacesmartcard.transmit_buffer[3]=0x00; //гу

	int i =0;
		switch(number)
		{
		case 0:
				{
					printf("key (для пароля): \n");
					for(i=0; i < 8;i++)
					{
						printf("%d) 0x%02hhx\n",i+1,key[i]);
					}
					cinterfacesmartcard.transmit_buffer[4]=0x08; //менты
					break;
				}
		case 1:
				{
					printf("key (для ключа без признака спецмер): \n");
					for(i=0; i < 32;i++)
					{
						printf("%d) 0x%02hhx\n",i+1,key[i]);
					}
					cinterfacesmartcard.transmit_buffer[4]=0x20; //менты
					break;
				}
		case 2:
				{
					printf("key (для ключа с признаком спецмер): \n");
					for(i=0; i < 36;i++)
					{
						printf("%d) 0x%02hhx\n",i+1,key[i]);
					}
					cinterfacesmartcard.transmit_buffer[4]=0x24; //менты
					break;
				}
		}


		write_byte(&ccomanda,5,&cinterfacesmartcard,numer_sim);
		while(cinterfacesmartcard.ok==0){}
	    cinterfacesmartcard.ok=0;


	    if(cinterfacesmartcard.reciver_buffer[0] == cinterfacesmartcard.transmit_buffer[1])
	    				{
	    					printf("Loading comand \"verify\"... \n");
	    				}
	    				else
	    				{
	    					if(cinterfacesmartcard.reciver_buffer[0] == 0x6D)
	    					{
	    						if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
	    						{
	    							printf("Error comand \"verify\" : 6D 00 (Неизвестная команда) \n");
	    						}
	    					}

	    					if(cinterfacesmartcard.reciver_buffer[0] == 0x67)
	    					{
	    						if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
	    						{
	    						    printf("Error comand \"verify\" : 67 00 (Неверная длина данных) \n");
	    						}
	    					}
	    				}


	    			switch(number)
	    			{
	    				case 0:
	    					{
	    						cinterfacesmartcard.transmit_buffer[0]= key[0] ;
	    						cinterfacesmartcard.transmit_buffer[1]= key[1] ;
	    						cinterfacesmartcard.transmit_buffer[2]= key[2] ;
	    						cinterfacesmartcard.transmit_buffer[3]= key[3] ;
	    						cinterfacesmartcard.transmit_buffer[4]= key[4] ;
	    						cinterfacesmartcard.transmit_buffer[5]= key[5] ;
	    						cinterfacesmartcard.transmit_buffer[6]= key[6] ;
	    						cinterfacesmartcard.transmit_buffer[7]= key[7] ;

	    						write_byte(&ccomanda,8,&cinterfacesmartcard,numer_sim);
	    						while(cinterfacesmartcard.ok==0){}
	    						cinterfacesmartcard.ok=0;

	    						break;
	    					}
	    				case 1:
	    					{

	    						int i=0;

	    						for( i =0; i < 32;i++)
	    						{
	    							cinterfacesmartcard.transmit_buffer[i]= key[i] ;
	    						}

	    						write_byte(&ccomanda,32,&cinterfacesmartcard,numer_sim);
	    						while(cinterfacesmartcard.ok==0){}
	    						cinterfacesmartcard.ok=0;

	    						break;

	    					}
	    				case 2:
	    					{
	    						int i=0;

	    						for( i =0; i < 36;i++)
	    						{
	    							cinterfacesmartcard.transmit_buffer[i]= key[i] ;
	    						}

	    						write_byte(&ccomanda,36,&cinterfacesmartcard,numer_sim);
	    						while(cinterfacesmartcard.ok==0){}
	    						cinterfacesmartcard.ok=0;

	    						break;
	    					}
	    			}

	    			if(cinterfacesmartcard.reciver_buffer[0] == 0x90)
	    			{
	    				if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
	    				{
	    					printf("End comand \"verify\" : Good = 90 00 \n");
	    				}
	    			}
	    			else
	    			{
	    				if(cinterfacesmartcard.reciver_buffer[0] == 0x63)
	    				{
	    					printf("Ошибка аутентификации или предъявления пароля: %d - число оставшихся попыток \n",cinterfacesmartcard.reciver_buffer[1]);
	    				}

	    				if(cinterfacesmartcard.reciver_buffer[0] == 0x69)
	    				{
	    					switch (cinterfacesmartcard.reciver_buffer[1])
	    					{
	    						case 0x82:
	    								{
	    									printf("Доступ не разрешен или ошибка предъявления транспортного кода \n : 69 82");
	    									break;
	    								}

	    						case 0x83:
	    								{
	    									printf("Ключ, пароль или транспортный код блокирован \n : 69 83");
	    									break;
	    								}
	    						case 0x85:
	    								{
	    									printf("Не выполнены условия использования команды (не выполнялись команды GET_CHALLENGE, GEN_SESSION_KEY, нет данных для GET_RESPONSE) \n : 69 85");
	    									break;
	    								}
	    						case 0x86:
	    								{
	    									printf("Команда не применима к данному типу файла \n : 69 86");
	    									break;
	    								}
	    						case 0xF0:
	    								{
	    									printf("Операция запрещена для данного ключа \n : 69 F0");
	    									break;
	    								}
	    						case 0xF2:
	    								{
	    									printf("В данной сессии не выработан диверсифицированный ключ \n : 69 F2");
	    									break;
	    								}

	    					}
	    				}

	    				if(cinterfacesmartcard.reciver_buffer[0] == 0x6A)
	    				{
	    					switch (cinterfacesmartcard.reciver_buffer[1])
	    					{
	    						case 0x83:
	    						{
	    							printf("Файл пуст \n : 6A 83");
	    							break;
	    						}
	    						case 0x86:
	    						{
	    							printf("Не верные параметры Р1 Р2 \n : 6A 86");
	    							break;
	    						}
	    						case 0x84:
	    						{
	    							printf("Не достаточно места для создания файла \n : 6A 84");
	    							break;
	    						}
	    					}
	    				}

	    			}

	    			printf("=========================\n");



		byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
		byt[1]=cinterfacesmartcard.reciver_buffer[2]; //
		byt[2]=cinterfacesmartcard.reciver_buffer[3]; //
		byt[3]=0; //
		somo_test=*qwd_byt;

		//Ошибка 63 С4 , где 4 это число оставшихся попыток

		//Если все попытки использовал то выдается ошибка 69 83 (Ключ, пароль или транспортный код блокирован;)


		return somo_test;
}

unsigned verify_tk(char tk_1, char tk_2, char tk_3, char tk_4, int numer_sim) //, unsigned tk_5, unsigned tk_6, unsigned tk_7, unsigned tk_8
{
	printf("=========================\n");
	printf("Start \"verify_tk\": \n");

	int somo_test;
	Ccomanda ccomanda;
	char byt[4];
	unsigned *qwd_byt;
	qwd_byt=(unsigned*)(byt);

	cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
	cinterfacesmartcard.transmit_buffer[1]=0x3e; //передаем номер инстртукции
	cinterfacesmartcard.transmit_buffer[2]=0x00; //ар
	cinterfacesmartcard.transmit_buffer[3]=0x00; //гу
	cinterfacesmartcard.transmit_buffer[4]=0x08; //менты
	write_byte(&ccomanda,5,&cinterfacesmartcard,numer_sim);
	while(cinterfacesmartcard.ok==0){}
    cinterfacesmartcard.ok=0;

    if(cinterfacesmartcard.reciver_buffer[0] == cinterfacesmartcard.transmit_buffer[1])
    {
    	printf("Loading comand \"verify_tk\"... \n");
    }
    else
    {
    	if(cinterfacesmartcard.reciver_buffer[0] == 0x6D)
    	{
    		if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
    		{
    			printf("Error comand \"verify_tk\" : 6D 00 (Неизвестная команда) \n");
    		}
    	}
    }

    cinterfacesmartcard.transmit_buffer[0]=tk_1;//0xAB;
	cinterfacesmartcard.transmit_buffer[1]=tk_2;//0x49;
	cinterfacesmartcard.transmit_buffer[2]=tk_3;//0xB8;
	cinterfacesmartcard.transmit_buffer[3]=tk_4;//0xF7;
	cinterfacesmartcard.transmit_buffer[4]=tk_1;//0xAB;
	cinterfacesmartcard.transmit_buffer[5]=tk_2;//0x49;
	cinterfacesmartcard.transmit_buffer[6]=tk_3;//0xB8;
	cinterfacesmartcard.transmit_buffer[7]=tk_4;//0xF7;

	write_byte(&ccomanda,8,&cinterfacesmartcard,numer_sim);
	while(cinterfacesmartcard.ok==0){}
	cinterfacesmartcard.ok=0;


	if(cinterfacesmartcard.reciver_buffer[0] == 0x90)
	{
		if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
		{
			printf("End comand \"verify_tk\" : Good = 90 00 \n");
		}
	}
	printf("=========================\n");



	byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
	byt[1]=cinterfacesmartcard.reciver_buffer[2]; //
	byt[2]=cinterfacesmartcard.reciver_buffer[3]; //
	byt[3]=0; //
	somo_test=*qwd_byt;



	return somo_test;
}

void SetSmartCart()
{
	int status;
	Ccomanda ccomanda;
	status=Xil_In32(baseaddress+0x238);// тип прерывания
	if(status==1)//приёмник
	{
			cinterfacesmartcard.reciver_buffer[cinterfacesmartcard.index_rec_buff]=Xil_In32(XPAR_HEAD_IP_0_BASEADDR+0x208);
			Xil_Out32(XPAR_HEAD_IP_0_BASEADDR+0x100, 4101);
		    Xil_Out32(XPAR_HEAD_IP_0_BASEADDR+0x100, 1048581);
			invert_byte(&cinterfacesmartcard.reciver_buffer[cinterfacesmartcard.index_rec_buff]);
			cinterfacesmartcard.index_rec_buff++;
	}
	if(status==2)//передатчик
	{
		Xil_Out32(baseaddress+0x100, 4101);
		Xil_Out32(XPAR_HEAD_IP_0_BASEADDR+0x100, 1048581);
		return;
	}
	if(status==3)// окончание приёма, сброс
	{
		Xil_Out32(XPAR_HEAD_IP_0_BASEADDR+0x100, 4101);
		Xil_Out32(baseaddress+0x228,50000000);// установка ожидание окончания 1 сек
		cinterfacesmartcard.number_read_byte=cinterfacesmartcard.index_rec_buff;
		cinterfacesmartcard.index_rec_buff=0;
		cinterfacesmartcard.initcil=0;
		cinterfacesmartcard.ok=1;
		return;
	}

}

void SetSmartCart_1()
{
	int status;
	Ccomanda ccomanda;
	status=Xil_In32(baseaddress_smoll_card+0x238);// тип прерывания
	if(status==1)//приёмник
	{
			cinterfacesmartcard.reciver_buffer[cinterfacesmartcard.index_rec_buff]=Xil_In32(XPAR_HEAD_IP_1_BASEADDR+0x208);
			Xil_Out32(XPAR_HEAD_IP_1_BASEADDR+0x100, 4101);
		    Xil_Out32(XPAR_HEAD_IP_1_BASEADDR+0x100, 1048581);
			invert_byte(&cinterfacesmartcard.reciver_buffer[cinterfacesmartcard.index_rec_buff]);
			cinterfacesmartcard.index_rec_buff++;
	}
	if(status==2)//передатчик
	{
		Xil_Out32(baseaddress_smoll_card+0x100, 4101);
		Xil_Out32(XPAR_HEAD_IP_1_BASEADDR+0x100, 1048581);
		return;
	}
	if(status==3)// окончание приёма, сброс
	{
		Xil_Out32(XPAR_HEAD_IP_1_BASEADDR+0x100, 4101);
		Xil_Out32(baseaddress_smoll_card+0x228,50000000);// установка ожидание окончания 1 сек
		cinterfacesmartcard.number_read_byte=cinterfacesmartcard.index_rec_buff;
		cinterfacesmartcard.index_rec_buff=0;
		cinterfacesmartcard.initcil=0;
		cinterfacesmartcard.ok=1;
		return;
	}

}


/*
unsigned testFS()
{
	int somo_test;
	Ccomanda ccomanda;
	char byt[4];
	unsigned *qwd_byt;
	qwd_byt=(unsigned*)(byt);
	cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
	cinterfacesmartcard.transmit_buffer[1]=0xF0; //передаем номер инстртукции
	cinterfacesmartcard.transmit_buffer[2]=0x00; //ар
	cinterfacesmartcard.transmit_buffer[3]=0x00; //гу
	cinterfacesmartcard.transmit_buffer[4]=0x04; //менты
	write_byte(&ccomanda,5,&cinterfacesmartcard);
	while(cinterfacesmartcard.ok==0){}
	cinterfacesmartcard.ok=0;
	byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
	byt[1]=cinterfacesmartcard.reciver_buffer[2]; //
	byt[2]=cinterfacesmartcard.reciver_buffer[3]; //
	byt[3]=0; //
	somo_test=*qwd_byt;
	return somo_test;
}
*/
unsigned create_df(char* id,char AC, int numer_sim)
{
	printf("=========================\n");
	printf("Start \"create_df\": \n");
	printf("ID: 0x%02hhx 0x%02hhx\n", id[0],id[1]);
	printf("Byt18: 0x00 (байт статуса)\n");
	printf("AC: \n");
	printf("0x%02hhx\n",AC);
	int i =0;

	for(i=0; i < 7;i++)
	{
		printf("0xFF\n");
	}




	int somo_test;
	Ccomanda ccomanda;
	char byt[4];
	unsigned *qwd_byt;
	qwd_byt=(unsigned*)(byt);
	cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
	cinterfacesmartcard.transmit_buffer[1]=0x32; //передаем номер инстртукции
	cinterfacesmartcard.transmit_buffer[2]=0x00; //ар
	cinterfacesmartcard.transmit_buffer[3]=0x00;
	cinterfacesmartcard.transmit_buffer[4]=0x0b; //гу
	write_byte(&ccomanda,5,&cinterfacesmartcard,numer_sim);
	while(cinterfacesmartcard.ok==0){}
	cinterfacesmartcard.ok=0;


	if(cinterfacesmartcard.reciver_buffer[0] == cinterfacesmartcard.transmit_buffer[1])
	{
		printf("Loading comand \"create_df\"... \n");
	}
	else
	{
		if(cinterfacesmartcard.reciver_buffer[0] == 0x6D)
		{
			if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
			{
				printf("Error comand \"create_df\" : 6D 00 (Неизвестная команда) \n");
			}
		}

		if(cinterfacesmartcard.reciver_buffer[0] == 0xff)
		{
			printf("Error comand \"create_df\" : ff (Неизвестная команда, карта заблокированна для выполнения команд) \n");
		}

		if(cinterfacesmartcard.reciver_buffer[0] == 0x69)
				{
					switch (cinterfacesmartcard.reciver_buffer[1])
					{
						case 0x82:
						{
							printf("Доступ не разрешен или ошибка предъявления транспортного кода \n : 69 82");
							break;
						}

						case 0x83:
						{
							printf("Ключ, пароль или транспортный код блокирован \n : 69 83");
							break;
						}
						case 0x85:
						{
							printf("Не выполнены условия использования команды (не выполнялись команды GET_CHALLENGE, GEN_SESSION_KEY, нет данных для GET_RESPONSE) \n : 69 85");
							break;
						}
						case 0x86:
						{
							printf("Команда не применима к данному типу файла \n : 69 86");
							break;
						}
						case 0xF0:
						{
							printf("Операция запрещена для данного ключа \n : 69 F0");
							break;
						}
						case 0xF2:
						{
							printf("В данной сессии не выработан диверсифицированный ключ \n : 69 F2");
							break;
						}

					}
				}
	}


	cinterfacesmartcard.transmit_buffer[0]=id[0];
	cinterfacesmartcard.transmit_buffer[1]=id[1];
	write_byte(&ccomanda,2,&cinterfacesmartcard,numer_sim);
	while(cinterfacesmartcard.ok==0){}
	cinterfacesmartcard.ok=0;


	cinterfacesmartcard.transmit_buffer[0]=0x00;
	write_byte(&ccomanda,1,&cinterfacesmartcard,numer_sim);
	while(cinterfacesmartcard.ok==0){}
	cinterfacesmartcard.ok=0;

	cinterfacesmartcard.transmit_buffer[0]=AC;
	cinterfacesmartcard.transmit_buffer[1]=0xFF;
	cinterfacesmartcard.transmit_buffer[2]=0xFF;
	cinterfacesmartcard.transmit_buffer[3]=0xFF;
	cinterfacesmartcard.transmit_buffer[4]=0xFF;
	cinterfacesmartcard.transmit_buffer[5]=0xFF;
	cinterfacesmartcard.transmit_buffer[6]=0xFF;
	cinterfacesmartcard.transmit_buffer[7]=0xFF;
	write_byte(&ccomanda,8,&cinterfacesmartcard,numer_sim);
	while(cinterfacesmartcard.ok==0){}
	cinterfacesmartcard.ok=0;



	if(cinterfacesmartcard.reciver_buffer[0] == 0x90)
	{
		if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
		{
			printf("End comand \"create_df\" : Good = 90 00 \n");
		}
	}
	else
	{
		if(cinterfacesmartcard.reciver_buffer[0] == 0x63)
		{
			printf("Ошибка аутентификации или предъявления пароля: %d - число оставшихся попыток \n",cinterfacesmartcard.reciver_buffer[1]);
		}

		if(cinterfacesmartcard.reciver_buffer[0] == 0x69)
		{
			switch (cinterfacesmartcard.reciver_buffer[1])
			{
				case 0x82:
				{
					printf("Доступ не разрешен или ошибка предъявления транспортного кода \n : 69 82");
					break;
				}

				case 0x83:
				{
					printf("Ключ, пароль или транспортный код блокирован \n : 69 83");
					break;
				}
				case 0x85:
				{
					printf("Не выполнены условия использования команды (не выполнялись команды GET_CHALLENGE, GEN_SESSION_KEY, нет данных для GET_RESPONSE) \n : 69 85");
					break;
				}
				case 0x86:
				{
					printf("Команда не применима к данному типу файла \n : 69 86");
					break;
				}
				case 0xF0:
				{
					printf("Операция запрещена для данного ключа \n : 69 F0");
					break;
				}
				case 0xF2:
				{
					printf("В данной сессии не выработан диверсифицированный ключ \n : 69 F2");
					break;
				}

			}
		}

										if(cinterfacesmartcard.reciver_buffer[0] == 0x6A)
										{
											switch (cinterfacesmartcard.reciver_buffer[1])
											{
												case 0x83:
												{
													printf("Файл пуст \n : 6A 83");
													break;
												}
												case 0x86:
												{
													printf("Не верные параметры Р1 Р2 \n : 6A 86");
													break;
												}
												case 0x84:
												{
													printf("Не достаточно места для создания файла \n : 6A 84");
													break;
												}
												case 0x80:
												{
													printf("Неприемлемое значение поля данных (файл с указанным идентификатором уже присутствует) \n : 6A 80");
													break;
												}
											}
										}

										if(cinterfacesmartcard.reciver_buffer[0] == 0xff)
										{
											printf("Error comand \"create_df\" : ff (Неизвестная команда, карта заблокированна для выполнения команд) \n");
										}
	}
	printf("=========================\n");


	byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
	byt[1]=cinterfacesmartcard.reciver_buffer[2]; //
	byt[2]=cinterfacesmartcard.reciver_buffer[3]; //
	byt[3]=0; //
	somo_test=*qwd_byt;
	return somo_test;
}

void DetectSmartCart()
{
	int status;
	status=Xil_In32(0x43C00008);
	Xil_Out32(0x43C00004,1);
	Xil_Out32(0x43C00004,0);
}


int SetUpInterruptSystem(XScuGic *XScuGicInstancePtr) //инициализация контроллера прерываний
{
	Xil_ExceptionRegisterHandler(XIL_EXCEPTION_ID_INT,
	(Xil_ExceptionHandler) XScuGic_InterruptHandler,
	XScuGicInstancePtr);
	Xil_ExceptionEnable();
	return XST_SUCCESS;
}


int ScuGicInterrupt_Init(u16 DeviceId,ProgSmartCart *pProgSmartCart, ProgSmartCart_1 *pProgSmartCart_1)
{
	int Status;
	XScuGic_Config *GicConfig;
	GicConfig = XScuGic_LookupConfig(DeviceId);
	if (NULL == GicConfig) {
	return XST_FAILURE;
	}
	Status = XScuGic_CfgInitialize(&InterruptController, GicConfig, GicConfig->CpuBaseAddress);
	if (Status != XST_SUCCESS) {
	return XST_FAILURE;
	}
	Status = SetUpInterruptSystem(&InterruptController);
	if (Status != XST_SUCCESS) {
	return XST_FAILURE;
	}
	//---схема управления
	Status = XScuGic_Connect(&InterruptController,61,SetSmartCart,pProgSmartCart);
	if (Status != XST_SUCCESS)
	{
		return XST_FAILURE;
	}
	XScuGic_Enable(&InterruptController, 61);


	Status = XScuGic_Connect(&InterruptController,62,SetSmartCart_1,pProgSmartCart_1);
    if (Status != XST_SUCCESS)
	{
		return XST_FAILURE;
	}
	XScuGic_Enable(&InterruptController, 62);
 	return XST_SUCCESS;
}

unsigned Load_FileSystem_new(char* password_KF12,char* password_key_KF10,int numer_sim)
{
	//*********Cоздаем файловую систему********************
	char id[2]; //2 байта ID файла DF после MF
	id[0] = 0x00; //1 байт
	id[1] =	0x00; //2 байт
	char AC_df = 0x00; // Условия доступа к DF  (если 00 то можно создавать KF и EF)

	select(id[0],id[1],numer_sim); // SELECT - команда перехода на файл ( принимает индификатор файла и номер симкарты)
	//Проверка условий существования файла DF с индификатором 0000
	if((cinterfacesmartcard.reciver_buffer[0] == 0x6A) && (cinterfacesmartcard.reciver_buffer[1] == 0x82))
	{
		//Если файл не существует, то создать файл DF
		printf("Создаем файл DF id = 0x%02hhx 0x%02hhx \n",id[0],id[1]);
		create_df(id,AC_df,numer_sim); // CREATE_DF - команда создания DF файла (принимает индификатор файла, условия достпупа, симкарту на какой создать)
	}
	else
	{
		//Если файл  существует, то после выполнения команды SELECT  перейдем на  файл DF
		printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id[0],id[1]);
	}


    //////////////////////////////////////
	//Насйтроки создания и првоерки существования файлк KF13 (файл ключа аутентификации)
    char AC_kf_13[4]; // условия доступа KF13
    AC_kf_13[0] = 0x22; // можно выполнять команду "Загрузка"(LOAD_KEY) "Смена,запись диверсифицированного ключа"(UPDATE_KEY,SAVE_DIV_KEY) только после verify KF FF12
    AC_kf_13[1] = 0x00; // можно выполнять команду (Внешняя аутентификация,взаимная аутентификация,предъявление пароля) (Внутренняя аутентификация)
    AC_kf_13[2] = 0xF0; // нельзя выполнять команду (Безвозвратное блокирование), можно выполнять команду (Разблокирование)
    AC_kf_13[3] = 0xF0; // нельзя выполнять команду (Диверсификация), можно выполнять команду (Выполнения шифрования или выработки имитовставки)

    select(0xFF,0x013,numer_sim); // переход на KF13 для проверки его сущствования
    //проверка существования KF13
    if((cinterfacesmartcard.reciver_buffer[0] == 0x6A) && (cinterfacesmartcard.reciver_buffer[1] == 0x82))
    {
    	printf("Создаем файл KF id = 0xFF 0x11 \n");
    	create_kf(0x13,AC_kf_13,0xF0,0x0D,numer_sim); // 1)индификатор; 2)условия доступа; 3) количество попыток( 2 тетрада); 3)условия байта 18; 4) номер симкарты
    	printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id[0],id[1]);
    	select(id[0],id[1],numer_sim);//переход обратно на DF 0x00 0x00
    }
    else
    {
    	printf("Файл  KF с id = 0xFF 0x13 существует \n");
    	printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id[0],id[1]);
    	select(id[0],id[1],numer_sim);//переход обратно на DF 0x00 0x00
    }

    //////////////////////////////////


    //////////////////////////////////
    //Насйтроки создания и првоерки существования файлк KF12 (Создание файла пароля)
    char AC_kf_12[4];
    AC_kf_12[0] = 0x0F; // можно выполнять команду "Загрузка"(LOAD_KEY), Нельзя выполнить команду "Смена,запись диверсифицированного ключа"(UPDATE_KEY,SAVE_DIV_KEY)
    AC_kf_12[1] = 0x0F; // можно выполнять команду (Внешняя аутентификация,взаимная аутентификация,предъявление пароля), нельзя (Внутренняя аутентификация)
    AC_kf_12[2] = 0x00; //можно выполнять команду (Безвозвратное блокирование), можно выполнять команду (Разблокирование)
    AC_kf_12[3] = 0xFF; // нельзя выполнять команду (Диверсификация), нельзя выполнять команду (Выполнения шифрования или выработки имитовставки)


   // char password[8];


    select(0xFF,0x012,numer_sim);
    if((cinterfacesmartcard.reciver_buffer[0] == 0x6A) && (cinterfacesmartcard.reciver_buffer[1] == 0x82))
    {
    	printf("Создаем файл KF id = 0xFF 0x12 \n");
    	create_kf(0x12,AC_kf_12,0x40,0x00,numer_sim); // создание KF Файла c ID: 3F12


    	/*
        password[0] = 0x8E;
        password[1] = 0xD5;
        password[2] = 0xD6;
        password[3] = 0x3D;
        password[4] = 0xBB;
        password[5] = 0xA2;
        password[6] = 0x21;
        password[7] = 0x36;

       	load_key(password,0,numer_sim); // Загрузка пароля в KF FF 12
        */

    	load_key(password_KF12,0,numer_sim); // Загрузка пароля в KF FF 12

    	printf("Перешли на файл DF id = 0x00 0x00 \n");
    	select(id[0],id[1],numer_sim);//переход обратно на DF 0x00 0x08
    }
    else
    {
    	printf("Файл  KF с id = 0xFF 0x12 существует \n");
    	printf("Перешли на файл DF id = 0x00 0x00 \n");
    	select(id[0],id[1],numer_sim);//переход обратно на DF 0x00 0x08
    }

    //////////////////////////////////

    //////////////////////////////////
    //Насйтроки создания и првоерки существования файлк KF10 (Файл диверсификации ключа)

    char AC_kf_10[4];
    char password_key[32];
    AC_kf_10[0] = 0x2F; // можно выполнять команду "Загрузка"(LOAD_KEY) после verify KF FF12, Нельзя выполнить команду "Смена,запись диверсифицированного ключа"(UPDATE_KEY,SAVE_DIV_KEY)
    AC_kf_10[1] = 0xFF; // нельзя выполнять команду (Внешняя аутентификация,взаимная аутентификация,предъявление пароля), нельзя (Внутренняя аутентификация)
    AC_kf_10[2] = 0xF2; //нельзя выполнять команду (Безвозвратное блокирование), можно выполнять команду (Разблокирование) только после verify KF FF12
    AC_kf_10[3] = 0x2F; // можно выполнять команду (Диверсификация) только после verify KF FF12, нельзя выполнять команду (Выполнения шифрования или выработки имитовставки)


    select(0xFF,0x010,numer_sim);
    if((cinterfacesmartcard.reciver_buffer[0] == 0x6A) && (cinterfacesmartcard.reciver_buffer[1] == 0x82))
    {
    	printf("Создаем файл KF id = 0xFF 0x10 \n");
    	create_kf(0x10,AC_kf_10,0x20,0x03,numer_sim);

    	/*
    	password_key[0] = 0x8E;
    	password_key[1] = 0xD5;
    	password_key[2] = 0xD6;
    	password_key[3] = 0x3D;
    	password_key[4] = 0xBB;
    	password_key[5] = 0xA2;
    	password_key[6] = 0x21;
    	password_key[7] = 0x36;


    	           password_key[8] = 0x8E;
    	           password_key[9] = 0xD5;
    	           password_key[10] = 0xD6;
    	           password_key[11] = 0x3D;
    	           password_key[12] = 0xBB;
    	           password_key[13] = 0xA2;
    	           password_key[14] = 0x21;
    	           password_key[15] = 0x36;


    	           password_key[16] = 0x8E;
    	           password_key[17] = 0xD5;
    	           password_key[18] = 0xD6;
    	           password_key[19] = 0x3D;
    	           password_key[20] = 0xBB;
    	           password_key[21] = 0xA2;
    	           password_key[22] = 0x21;
    	           password_key[23] = 0x36;


    	           password_key[24] = 0x8E;
    	           password_key[25] = 0xD5;
    	           password_key[26] = 0xD6;
    	           password_key[27] = 0x3D;
    	           password_key[28] = 0xBB;
    	           password_key[29] = 0xA2;
    	           password_key[30] = 0x21;
    	           password_key[31] = 0x36;

    	           load_key(password_key,1,numer_sim); // Загрузка ключа в KF FF 10
    	           */

    	    	   load_key(password_key_KF10,1,numer_sim); // Загрузка ключа в KF FF 10

    	    	printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id[0],id[1]);
    	    	select(id[0],id[1],numer_sim);//переход обратно на DF 0x00 0x00
    	    }
    	    else
    	    {
    	    	printf("Файл  KF с id = 0xFF 0x10 существует \n");
    	    	printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id[0],id[1]);
    	    	select(id[0],id[1],numer_sim);//переход обратно на DF 0x00 0x00
    	    }


    	    /////////////////////////////////


  	    //////////////////////////////////////


  	 	 printf("Переход на KF файл : \n");
  	 	 select(0xFF,0x12,numer_sim);
  	 	 clean_cInterfaceSmartcard(&cinterfacesmartcard);

  	 	 int i =0;
  	 	 /*
  	 	 password[0] = 0x8E;
  	 	 password[1] = 0xD5;
  	 	 password[2] = 0xD6;
  	 	 password[3] = 0x3D;
  	 	 password[4] = 0xBB;
  	 	 password[5] = 0xA2;
  	 	 password[6] = 0x21;
  	 	 password[7] = 0x36;
  	 	 */

  	 	 for(i=0; i < 8;i++)
  	 	 {
  	 		 printf("0x%02hhx\n",password_KF12[i]);
  	 	 }

  	 	 printf("Проходим верификацию на клиенте: \n");
  	 	 verify(password_KF12,0,numer_sim);
  	 	 clean_cInterfaceSmartcard(&cinterfacesmartcard);


  	 	 select(0xFF,0x13,numer_sim);


  	 	 char k[32];

  	 	 for(i=0; i < 32; i++)
  	 	 {
  	 		 k[i]=0x00;
  	 	 }

  	 	 load_key(k,1,numer_sim);



//////////////////////////////////

  	 	//*********Cоздания файловой системы (левая)********************

  	 	   select(id[0],id[1],numer_sim);//переход обратно на DF 0x00 0x00


 		   char AC_kf_11[4];
 		   AC_kf_11[0] = 0x0F; //0x0F
 		   AC_kf_11[1] = 0x0F;
 		   AC_kf_11[2] = 0xF0;
 		   AC_kf_11[3] = 0xFF;

 		   select(0xFF,0x011,numer_sim);
 		   if((cinterfacesmartcard.reciver_buffer[0] == 0x6A) && (cinterfacesmartcard.reciver_buffer[1] == 0x82))
 		   {
 		  	  printf("Создаем файл KF id = 0xFF 0x11 \n");
 		  	  create_kf(0x11,AC_kf_11,0x40,0x00,numer_sim);
 		  	  printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id[0],id[1]);
 		  	  select(id[0],id[1],numer_sim);//переход обратно на DF 0x00 0x00
 		   }
 		   else
 		   {
 		  	  printf("Файл  KF с id = 0xFF 0x11 существует \n");
 		  	  printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id[0],id[1]);
 		  	  select(id[0],id[1],numer_sim);//переход обратно на DF 0x00 0x00
 		   }

///////////////////////////////////////

 		   char AC_kf_14[4];
 		   AC_kf_14[0] = 0x11;
 		   AC_kf_14[1] = 0x0F;
 		   AC_kf_14[2] = 0xF1;
 		   AC_kf_14[3] = 0xFF;

 		   select(0xFF,0x014,numer_sim);
 		   if((cinterfacesmartcard.reciver_buffer[0] == 0x6A) && (cinterfacesmartcard.reciver_buffer[1] == 0x82))
 		   {
 			   printf("Создаем файл KF id = 0xFF 0x14 \n");
 			   create_kf(0x14,AC_kf_14,0x40,0x00,numer_sim);
 			   printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id[0],id[1]);
 			   select(id[0],id[1],numer_sim);//переход обратно на DF 0x00 0x00
 		   }
 		   else
 		   {
 			   printf("Файл  KF с id = 0xFF 0x14 существует \n");
 			   printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id[0],id[1]);
 			   select(id[0],id[1],numer_sim);//переход обратно на DF 0x00 0x08
 		   }

 		   /////////////////////////////


  	 		   char id_left_ef[2];
  	 		   id_left_ef[0] = 0x00;
  	 		   id_left_ef[1] = 0x01;
  	 		   char size_left_ef[2];
  	 		   size_left_ef[0]=0x00;
  	 		   size_left_ef[1]=0x02;
  	 		   char AC_left_ef = 0x44;



  	 		   select(id_left_ef[0],id_left_ef[1],numer_sim);

  	 		   if((cinterfacesmartcard.reciver_buffer[0] == 0x6A) && (cinterfacesmartcard.reciver_buffer[1] == 0x82))
  	 		   {
  	 		      printf("Создаем файл EF id = 0x%02hhx 0x%02hhx \n",id_left_ef[0],id_left_ef[1]);
  	 		      create_ef(size_left_ef,id_left_ef,AC_left_ef,0x00,numer_sim);
  	 		      printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id[0],id[1]);
  	 		      select(id[0],id[1],numer_sim);//переход обратно на DF 0x00 0x08
  	 		   }
  	 		   else
  	 		   {
  	 		      printf("Файл  EF с id = 0x%02hhx 0x%02hhx существует \n",id_left_ef[0],id_left_ef[1]);
  	 		      printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id[0],id[1]);
  	 		      select(id[0],id[1],numer_sim);//переход обратно на DF 0x00 0x08
  	 		   }

  	 		 /////////////////////////////

  	 		   char id_left_ef_2[2];
  	 		   id_left_ef_2[0] = 0x00;
  	 		   id_left_ef_2[1] = 0x02;
  	 		   char size_left_ef_2[2];
  	 		   size_left_ef_2[0]=0x00;
  	 		   size_left_ef_2[1]=0x04;
  	 		   char AC_left_ef_2 = 0x44;



  	 		   select(id_left_ef_2[0],id_left_ef_2[1],numer_sim);

  	 		   if((cinterfacesmartcard.reciver_buffer[0] == 0x6A) && (cinterfacesmartcard.reciver_buffer[1] == 0x82))
  	 		   {
  	 		      printf("Создаем файл EF id = 0x%02hhx 0x%02hhx \n",id_left_ef_2[0],id_left_ef_2[1]);
  	 		      create_ef(size_left_ef_2,id_left_ef_2,AC_left_ef_2,0x00,numer_sim);
  	 		      printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id[0],id[1]);
  	 		      select(id[0],id[1],numer_sim);//переход обратно на DF 0x00 0x08
  	 		   }
  	 		   else
  	 		   {
  	 		      printf("Файл  EF с id = 0x%02hhx 0x%02hhx существует \n",id_left_ef_2[0],id_left_ef_2[1]);
  	 		      printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id[0],id[1]);
  	 		      select(id[0],id[1],numer_sim);//переход обратно на DF 0x00 0x08
  	 		   }



  	 		   char id_left_ef_3[2];
  	 		   id_left_ef_3[0] = 0x00;
  	 		   id_left_ef_3[1] = 0x03;
  	 		   char size_left_ef_3[2];
  	 		   size_left_ef_3[0]=0x00;
  	 		   size_left_ef_3[1]=0x02;
  	 		   char AC_left_ef_3 = 0x44;



  	 		   select(id_left_ef_3[0],id_left_ef_3[1],numer_sim);

  	 		   if((cinterfacesmartcard.reciver_buffer[0] == 0x6A) && (cinterfacesmartcard.reciver_buffer[1] == 0x82))
  	 		   {
  	 		      printf("Создаем файл EF id = 0x%02hhx 0x%02hhx \n",id_left_ef_3[0],id_left_ef_3[1]);
  	 		      create_ef(size_left_ef_3,id_left_ef_3,AC_left_ef_3,0x00,numer_sim);
  	 		      printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id[0],id[1]);
  	 		      select(id[0],id[1],numer_sim);//переход обратно на DF 0x00 0x08
  	 		   }
  	 		   else
  	 		   {
  	 		      printf("Файл  EF с id = 0x%02hhx 0x%02hhx существует \n",id_left_ef_3[0],id_left_ef_3[1]);
  	 		      printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id[0],id[1]);
  	 		      select(id[0],id[1],numer_sim);//переход обратно на DF 0x00 0x08
  	 		   }


  	 		//*********Конец Cоздания файловой системы (левая)********************


}

//Вроде эта последняя версия зашифки файловой системы
unsigned Load_FileSystem(int numer_sim)
{
	//*********Cоздаем файловую систему (правая)********************
	    char id[2];
	    id[0] = 0x00;
	    id[1] =	0x04;
	    char AC_df = 0x00;

	    select(id[0],id[1],numer_sim);
	    if((cinterfacesmartcard.reciver_buffer[0] == 0x6A) && (cinterfacesmartcard.reciver_buffer[1] == 0x82))
	    {
	    	printf("Создаем файл DF id = 0x%02hhx 0x%02hhx \n",id[0],id[1]);
	    	create_df(id,AC_df,numer_sim);
	    }
	    else
	    {
	    	printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id[0],id[1]);
	    }


	    //////////////////////////////
        char AC_ef = 0x01;
	    char id_ef[2];
	    id_ef[0] = 0x00;
	    id_ef[1] = 0x05; //0x09
	    char size_ef[2];
	    size_ef[0]=0x00;
	    size_ef[1]=0x02;



	    select(id_ef[0],id_ef[1],numer_sim);
	    if((cinterfacesmartcard.reciver_buffer[0] == 0x6A) && (cinterfacesmartcard.reciver_buffer[1] == 0x82))
	    {
	    	printf("Создаем файл EF id = 0x%02hhx 0x%02hhx \n",id_ef[0],id_ef[1]);
	    	create_ef(size_ef,id_ef,AC_ef,0x00,numer_sim);
	    	printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id[0],id[1]);
	    	select(id[0],id[1],numer_sim);//переход обратно на DF 0x00 0x08
	    }
	    else
	    {
	    	printf("Файл  EF с id = 0x00 0x09 существует \n");
	    	printf("Перешли на файл DF id = 0x00 0x08 \n");
	    	select(id[0],id[1],numer_sim);//переход обратно на DF 0x00 0x08
	    }

	    //////////////////////////////////////

	    char AC_kf_11[4];
	    AC_kf_11[0] = 0x02;
	    AC_kf_11[1] = 0x00;
	    AC_kf_11[2] = 0xF0;
	    AC_kf_11[3] = 0xFF;

	    select(0xFF,0x011,numer_sim);
	    if((cinterfacesmartcard.reciver_buffer[0] == 0x6A) && (cinterfacesmartcard.reciver_buffer[1] == 0x82))
	    {
	    	printf("Создаем файл KF id = 0xFF 0x11 \n");
	    	create_kf(0x11,AC_kf_11,0xF0,0x3D,numer_sim);
	    	printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id[0],id[1]);
	    	select(id[0],id[1],numer_sim);//переход обратно на DF 0x00 0x08
	    }
	    else
	    {
	    	printf("Файл  KF с id = 0xFF 0x11 существует \n");
	    	printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id[0],id[1]);
	    	select(id[0],id[1],numer_sim);//переход обратно на DF 0x00 0x08
	    }

	    //////////////////////////////////

	    char AC_kf_12[4];
	    char password[8];
	    AC_kf_12[0] = 0x00; //0x0F
	    AC_kf_12[1] = 0x0F;
	    AC_kf_12[2] = 0x00;
	    AC_kf_12[3] = 0xFF;


	    select(0xFF,0x012,numer_sim);
	    if((cinterfacesmartcard.reciver_buffer[0] == 0x6A) && (cinterfacesmartcard.reciver_buffer[1] == 0x82))
	    {
	    	printf("Создаем файл KF id = 0xFF 0x12 \n");
	    	create_kf(0x12,AC_kf_12,0x40,0x00,numer_sim);



	        password[0] = 0x8E;
	        password[1] = 0xD5;
	        password[2] = 0xD6;
	        password[3] = 0x3D;
	        password[4] = 0xBB;
	        password[5] = 0xA2;
	        password[6] = 0x21;
	        password[7] = 0x36;

	        load_key(password,0,numer_sim); // Загрузка пароля в KF FF 12

	    	printf("Перешли на файл DF id = 0x00 0x08 \n");
	    	select(id[0],id[1],numer_sim);//переход обратно на DF 0x00 0x08
	    }
	    else
	    {
	    	printf("Файл  KF с id = 0xFF 0x12 существует \n");
	    	printf("Перешли на файл DF id = 0x00 0x08 \n");
	    	select(id[0],id[1],numer_sim);//переход обратно на DF 0x00 0x08
	    }

	    //////////////////////////////////


	   char AC_kf_10[4];
	    char password_key[32];
	    AC_kf_10[0] = 0x22; //0x2F
	    AC_kf_10[1] = 0xFF;
	    AC_kf_10[2] = 0xF2;
	    AC_kf_10[3] = 0x2F;


	    select(0xFF,0x010,numer_sim);
	    if((cinterfacesmartcard.reciver_buffer[0] == 0x6A) && (cinterfacesmartcard.reciver_buffer[1] == 0x82))
	    {
	    	printf("Создаем файл KF id = 0xFF 0x10 \n");
	    	create_kf(0x10,AC_kf_10,0x20,0x03,numer_sim);

	           password_key[0] = 0x8E;
	           password_key[1] = 0xD5;
	           password_key[2] = 0xD6;
	           password_key[3] = 0x3D;
	           password_key[4] = 0xBB;
	           password_key[5] = 0xA2;
	           password_key[6] = 0x21;
	           password_key[7] = 0x36;


	           password_key[8] = 0x8E;
	           password_key[9] = 0xD5;
	           password_key[10] = 0xD6;
	           password_key[11] = 0x3D;
	           password_key[12] = 0xBB;
	           password_key[13] = 0xA2;
	           password_key[14] = 0x21;
	           password_key[15] = 0x36;


	           password_key[16] = 0x8E;
	           password_key[17] = 0xD5;
	           password_key[18] = 0xD6;
	           password_key[19] = 0x3D;
	           password_key[20] = 0xBB;
	           password_key[21] = 0xA2;
	           password_key[22] = 0x21;
	           password_key[23] = 0x36;


	           password_key[24] = 0x8E;
	           password_key[25] = 0xD5;
	           password_key[26] = 0xD6;
	           password_key[27] = 0x3D;
	           password_key[28] = 0xBB;
	           password_key[29] = 0xA2;
	           password_key[30] = 0x21;
	           password_key[31] = 0x36;

	           load_key(password_key,1,numer_sim); // Загрузка ключа в KF FF 10


	    	printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id[0],id[1]);
	    	select(id[0],id[1],numer_sim);//переход обратно на DF 0x00 0x08
	    }
	    else
	    {
	    	printf("Файл  KF с id = 0xFF 0x10 существует \n");
	    	printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id[0],id[1]);
	    	select(id[0],id[1],numer_sim);//переход обратно на DF 0x00 0x08
	    }


	    /////////////////////////////////


	    char AC_kf_13[4];
	   	 	 	AC_kf_13[0] = 0x02;
	   		    AC_kf_13[1] = 0x00;
	   		    AC_kf_13[2] = 0xF0;
	   		    AC_kf_13[3] = 0xF0;

	   		    select(0xFF,0x013,numer_sim);
	   		    if((cinterfacesmartcard.reciver_buffer[0] == 0x6A) && (cinterfacesmartcard.reciver_buffer[1] == 0x82))
	   		    {
	   		    	printf("Создаем файл KF id = 0xFF 0x13 \n");
	   		    	create_kf(0x13,AC_kf_13,0xF0,0x0D,numer_sim);
	   		    	printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id[0],id[1]);
	   		    	select(id[0],id[1],0);//переход обратно на DF 0x00 0x08
	   		    }
	   		    else
	   		    {
	   		    	printf("Файл  KF с id = 0xFF 0x13 существует \n");
	   		    	printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id[0],id[1]);
	   		    	select(id[0],id[1],numer_sim);//переход обратно на DF 0x00 0x08
	   		    }

	   		    //////////////////////////////
	   		        char AC_ef1 = 0x03;
	   		   	    char id_ef1[2];
	   		   	    id_ef1[0] = 0x00;
	   		   	    id_ef1[1] = 0x06;
	   		   	    char size_ef1[2];
	   		   	    size_ef1[0]=0x00;
	   		   	    size_ef1[1]=0x10; //16 байт



	   		   	    select(id_ef1[0],id_ef1[1],numer_sim);
	   		   	    if((cinterfacesmartcard.reciver_buffer[0] == 0x6A) && (cinterfacesmartcard.reciver_buffer[1] == 0x82))
	   		   	    {
	   		   	    	printf("Создаем файл EF id = 0x%02hhx 0x%02hhx \n",id_ef1[0],id_ef1[1]);
	   		   	    	create_ef(size_ef1,id_ef1,AC_ef1,0x00,numer_sim);
	   		   	    	printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id[0],id[1]);
	   		   	    	select(id[0],id[1],0);//переход обратно на DF 0x00 0x08
	   		   	    }
	   		   	    else
	   		   	    {
	   		   	    	printf("Файл  EF с id = 0x00 0x06 существует \n");
	   		   	    	printf("Перешли на файл DF id = 0x00 0x08 \n");
	   		   	    	select(id[0],id[1],numer_sim);//переход обратно на DF 0x00 0x08
	   		   	    }

	   		   	    //////////////////////////////////////


	   		   	 printf("Переход на KF файл клиента: \n");
	   		   	 	 select(0xFF,0x12,numer_sim);
	   		   	 	 clean_cInterfaceSmartcard(&cinterfacesmartcard);

	   		   	 	 int i =0;
	   		   	 	 password[0] = 0x8E;
	   		   	 	 password[1] = 0xD5;
	   		   	 	 password[2] = 0xD6;
	   		   	 	 password[3] = 0x3D;
	   		   	 	 password[4] = 0xBB;
	   		   	 	 password[5] = 0xA2;
	   		   	 	 password[6] = 0x21;
	   		   	 	 password[7] = 0x36;

	   		   	 	 for(i=0; i < 8;i++)
	   		   	 	 {
	   		   	 		 printf("0x%02hhx\n",password[i]);
	   		   	 	 }

	   		   	 	 printf("Проходим верификацию на клиенте: \n");
	   		   	 	 verify(password,0,numer_sim);
	   		   	 	 clean_cInterfaceSmartcard(&cinterfacesmartcard);


	   		   	 	 select(0xFF,0x13,numer_sim);


	   		   	 	 char k[32];

	   		   	 	 for(i=0; i < 32; i++)
	   		   	 	 {
	   		   	 		 k[i]=0x00;
	   		   	 	 }

	   		   	 	 load_key(k,1,numer_sim);



	    //////////////////////////////////

	//*********Конец Cоздания файловой системы (правая)********************


	//*********Cоздания файловой системы (левая)********************

	    /*
	   char id_left_df[2];
	   id_left_df[0] = 0x00;
	   id_left_df[1] = 0x00;
	   char AC_left_df = 0x00;


	   select(id_left_df[0],id_left_df[1],numer_sim);
	   if((cinterfacesmartcard.reciver_buffer[0] == 0x6A) && (cinterfacesmartcard.reciver_buffer[1] == 0x82))
	   {
	   	printf("Создаем файл DF id = 0x%02hhx 0x%02hhx \n",id_left_df[0],id_left_df[1]);
	   	create_df(id_left_df,AC_left_df,numer_sim);
	   }
	   else
	   {
	   	printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id_left_df[0],id_left_df[1]);
	   }

	   select(id_left_df[0],id_left_df[1],numer_sim);


	   char id_left_ef[2];
	   id_left_ef[0] = 0x00;
	   id_left_ef[1] = 0x01;
	   char size_left_ef[2];
	   size_left_ef[0]=0x00;
	   size_left_ef[1]=0x02;
	   char AC_left_ef = 0x44;



	   select(id_left_ef[0],id_left_ef[1],numer_sim);

	   if((cinterfacesmartcard.reciver_buffer[0] == 0x6A) && (cinterfacesmartcard.reciver_buffer[1] == 0x82))
	   {
	      printf("Создаем файл EF id = 0x%02hhx 0x%02hhx \n",id_left_ef[0],id_left_ef[1]);
	      create_ef(size_left_ef,id_left_ef,AC_left_ef,0x00,numer_sim);
	      printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id_left_df[0],id_left_df[1]);
	      select(id_left_df[0],id_left_df[1],numer_sim);//переход обратно на DF 0x00 0x08
	   }
	   else
	   {
	      printf("Файл  EF с id = 0x%02hhx 0x%02hhx существует \n",id_left_ef[0],id_left_ef[1]);
	      printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id_left_df[0],id_left_df[1]);
	      select(id_left_df[0],id_left_df[1],numer_sim);//переход обратно на DF 0x00 0x08
	   }


	   char id_left_ef_2[2];
	   id_left_ef_2[0] = 0x00;
	   id_left_ef_2[1] = 0x02;
	   char size_left_ef_2[2];
	   size_left_ef_2[0]=0x00;
	   size_left_ef_2[1]=0x02;
	   char AC_left_ef_2 = 0x44;



	   select(id_left_ef_2[0],id_left_ef_2[1],numer_sim);

	   if((cinterfacesmartcard.reciver_buffer[0] == 0x6A) && (cinterfacesmartcard.reciver_buffer[1] == 0x82))
	   {
	      printf("Создаем файл EF id = 0x%02hhx 0x%02hhx \n",id_left_ef_2[0],id_left_ef_2[1]);
	      create_ef(size_left_ef_2,id_left_ef_2,AC_left_ef_2,0x00,numer_sim);
	      printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id_left_df[0],id_left_df[1]);
	      select(id_left_df[0],id_left_df[1],numer_sim);//переход обратно на DF 0x00 0x08
	   }
	   else
	   {
	      printf("Файл  EF с id = 0x%02hhx 0x%02hhx существует \n",id_left_ef_2[0],id_left_ef_2[1]);
	      printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id_left_df[0],id_left_df[1]);
	      select(id_left_df[0],id_left_df[1],numer_sim);//переход обратно на DF 0x00 0x08
	   }



	   char id_left_ef_3[2];
	   id_left_ef_3[0] = 0x00;
	   id_left_ef_3[1] = 0x03;
	   char size_left_ef_3[2];
	   size_left_ef_3[0]=0x00;
	   size_left_ef_3[1]=0x02;
	   char AC_left_ef_3 = 0x44;



	   select(id_left_ef_3[0],id_left_ef_3[1],numer_sim);

	   if((cinterfacesmartcard.reciver_buffer[0] == 0x6A) && (cinterfacesmartcard.reciver_buffer[1] == 0x82))
	   {
	      printf("Создаем файл EF id = 0x%02hhx 0x%02hhx \n",id_left_ef_3[0],id_left_ef_3[1]);
	      create_ef(size_left_ef_3,id_left_ef_3,AC_left_ef_3,0x00,numer_sim);
	      printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id_left_df[0],id_left_df[1]);
	      select(id_left_df[0],id_left_df[1],numer_sim);//переход обратно на DF 0x00 0x08
	   }
	   else
	   {
	      printf("Файл  EF с id = 0x%02hhx 0x%02hhx существует \n",id_left_ef_3[0],id_left_ef_3[1]);
	      printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id_left_df[0],id_left_df[1]);
	      select(id_left_df[0],id_left_df[1],numer_sim);//переход обратно на DF 0x00 0x08
	   }

	   char AC_kf_14[4];
	   AC_kf_14[0] = 0x33;
	   AC_kf_14[1] = 0x0F;
	   AC_kf_14[2] = 0xF3;
	   AC_kf_14[3] = 0xFF;

	   select(0xFF,0x014,numer_sim);
	   if((cinterfacesmartcard.reciver_buffer[0] == 0x6A) && (cinterfacesmartcard.reciver_buffer[1] == 0x82))
	   {
		   printf("Создаем файл KF id = 0xFF 0x14 \n");
		   create_kf(0x14,AC_kf_14,0x40,0x00,numer_sim);
		   printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id_left_df[0],id_left_df[1]);
		   select(id_left_df[0],id_left_df[1],numer_sim);//переход обратно на DF 0x00 0x08
	   }
	   else
	   {
		   printf("Файл  KF с id = 0xFF 0x14 существует \n");
		   printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id_left_df[0],id_left_df[1]);
		   select(id_left_df[0],id_left_df[1],numer_sim);//переход обратно на DF 0x00 0x08
	   }

	   char AC_kf_13[4];
	   AC_kf_13[0] = 0x00; //0x0F
	   AC_kf_13[1] = 0x0F;
	   AC_kf_13[2] = 0x00;
	   AC_kf_13[3] = 0xFF;

	   select(0xFF,0x013,numer_sim);
	   if((cinterfacesmartcard.reciver_buffer[0] == 0x6A) && (cinterfacesmartcard.reciver_buffer[1] == 0x82))
	   {
	  	  printf("Создаем файл KF id = 0xFF 0x13 \n");
	  	  create_kf(0x13,AC_kf_13,0x40,0x00,numer_sim);
	  	  printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id_left_df[0],id_left_df[1]);
	  	  select(id_left_df[0],id_left_df[1],numer_sim);//переход обратно на DF 0x00 0x08
	   }
	   else
	   {
	  	  printf("Файл  KF с id = 0xFF 0x13 существует \n");
	  	  printf("Перешли на файл DF id = 0x%02hhx 0x%02hhx \n",id_left_df[0],id_left_df[1]);
	  	  select(id_left_df[0],id_left_df[1],numer_sim);//переход обратно на DF 0x00 0x08
	   }

	   */
	//*********Конец Cоздания файловой системы (левая)********************


	   return 0;
}

unsigned Start_ayntificate(char* serial_numer)
{

	printf("Старт алгоритма аутентификации: \n");

	printf("Переход на Мастер файл базовой станции: \n");
	select(0x3F,0x00,0);

	printf("Переход на Мастер файл клиента: \n");
	select(0x3F,0x00,1);

	 char id[2];
	 int i=0;
	 id[0] = 	0x00;
	 id[1] =	0x04;// 0x08;

	 printf("Переход на DF файл клиента: \n");
	 select(id[0],id[1],1);
	 clean_cInterfaceSmartcard(&cinterfacesmartcard);
	 printf("Переход на KF файл клиента: \n");
	 select(0xFF,0x12,1);
	 clean_cInterfaceSmartcard(&cinterfacesmartcard);

	 char password[8];
	 password[0] = 0x8E;
	 password[1] = 0xD5;
	 password[2] = 0xD6;
	 password[3] = 0x3D;
	 password[4] = 0xBB;
	 password[5] = 0xA2;
	 password[6] = 0x21;
	 password[7] = 0x36;

	 for(i=0; i < 8;i++)
	 {
		 printf("0x%02hhx\n",password[i]);
	 }

	 printf("Проходим верификацию на клиенте: \n");
	 verify(password,0,1);
	 clean_cInterfaceSmartcard(&cinterfacesmartcard);

	 printf("Переход на KF файл клиента: \n");
	 select(0xFF,0x10,1);
	 clean_cInterfaceSmartcard(&cinterfacesmartcard);

	 printf("Производим диверсификацию ключа на клиенте: \n");
	 diversify_key(serial_numer,1);
	 clean_cInterfaceSmartcard(&cinterfacesmartcard);

	 printf("Сохраняем диверсифицированный ключ на клиенте: \n");
	 //save_div_key(0xFF,0x11,1);
	 save_div_key(0xFF,0x13,1);
	 clean_cInterfaceSmartcard(&cinterfacesmartcard);

	 printf("Переход на KF файл базовой станции: \n");
	 //select(0xFF,0x11,1);
	 select(0xFF,0x13,1);
	 clean_cInterfaceSmartcard(&cinterfacesmartcard);


	 /////////////////////////////////////////

	 printf("Переход на DF файл базовой станции: \n");
	 select(id[0],id[1],0);
	 clean_cInterfaceSmartcard(&cinterfacesmartcard);

	 printf("Переход на KF файл базовой станции: \n");
	 select(0xFF,0x12,0);
	 clean_cInterfaceSmartcard(&cinterfacesmartcard);


	 password[0] = 0x8E;
	 password[1] = 0xD5;
	 password[2] = 0xD6;
	 password[3] = 0x3D;
	 password[4] = 0xBB;
	 password[5] = 0xA2;
	 password[6] = 0x21;
	 password[7] = 0x36;

	 for(i=0; i < 8;i++)
	 {
		printf("0x%02hhx\n",password[i]);
	 }

	 printf("Проходим верификацию базовой станции: \n");
	 verify(password,0,0);
	 clean_cInterfaceSmartcard(&cinterfacesmartcard);

	 printf("Переход на KF файл базовой станции: \n");
	 select(0xFF,0x10,0);
	 clean_cInterfaceSmartcard(&cinterfacesmartcard);

	 printf("Диверсифицируем ключ на базовой станции: \n");
	 diversify_key(serial_numer,0);
	 clean_cInterfaceSmartcard(&cinterfacesmartcard);

	 printf("Сохраняем диверсифицируемый ключ в KF на базовой станции: \n");
	// save_div_key(0xFF,0x11,0);
	 save_div_key(0xFF,0x13,0);
	 clean_cInterfaceSmartcard(&cinterfacesmartcard);


	 printf("Переход на KF файл базовой станции: \n");
	 select(0xFF,0x13,0);
	 clean_cInterfaceSmartcard(&cinterfacesmartcard);

	 char* random =  malloc(8);

	 printf("Генирируем рандомное число на базовой станции: \n");
	 random = get_challenge(0x08,0);

	 printf("Проходим внутреннию аунтификацию на клиенте: \n");
	 internal_auth(random,1);
	 clean_cInterfaceSmartcard(&cinterfacesmartcard);

	 char* Q = malloc(6);
	 printf("Получаем данные от внутренней аунтификации на клиенте: \n");
	 Q = get_response(0x06,1);

	 printf("Q: \n");

	 for(i=0; i < 6;i++)
	 {
	  printf("0x%02hhx \n",Q[i]);
	 }

	clean_cInterfaceSmartcard(&cinterfacesmartcard);

	printf("Проводим внешняию аутентификацию на базовой станции: \n");
	external_authenticate(Q,0);

	printf("Переходим в EF на базовой станции: \n");
	select(0x00,0x06,0);

	char data[16];
	data[0] = 0x00;
	data[1] = 0x01;
	data[2] = 0x00;
	data[3] = 0x01;
	data[4] = 0x00;
	data[5] = 0x01;
	data[6] = 0x00;
	data[7] = 0x01;
	data[8] = 0x00;
	data[9] = 0x01;
	data[10] = 0x00;
	data[11] = 0x01;
	data[12] = 0x00;
	data[13] = 0x01;
	data[14] = 0x00;
	data[15] = 0x01;

	printf("Записываем данные в EF на базовой станции: \n");
	update_binary(data,0x10,0,0);

	return 0;
}

char* Crypt(char* data,char P1,char P2,char Lce,int numer_sim)
{
	printf("=========================\n");
	printf("Start \"Crypt\": \n");

	int somo_test;
	Ccomanda ccomanda;
	char* byt;

	cinterfacesmartcard.transmit_buffer[0]=0x80; //передаем класс команды
	cinterfacesmartcard.transmit_buffer[1]=0x54; //передаем номер инстртукции
	cinterfacesmartcard.transmit_buffer[2]=P1; //ар
	cinterfacesmartcard.transmit_buffer[3]=P2;
	cinterfacesmartcard.transmit_buffer[4]=Lce; //гу

	write_byte(&ccomanda,5,&cinterfacesmartcard,numer_sim);
	while(cinterfacesmartcard.ok==0){}
	cinterfacesmartcard.ok=0;


	if(cinterfacesmartcard.reciver_buffer[0] == cinterfacesmartcard.transmit_buffer[1])
	{
		printf("Loading comand \"Crypt\"... \n");
	}
	else
	{
		if(cinterfacesmartcard.reciver_buffer[0] == 0x6D)
		{
			if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
			{
				printf("Error comand \"Crypt\" : 6D 00 (Неизвестная команда) \n");
				}
			}

			if(cinterfacesmartcard.reciver_buffer[0] == 0xff)
			{
				printf("Error comand \"Crypt\" : ff (Неизвестная команда, карта заблокированна для выполнения команд) \n");
			}

			if(cinterfacesmartcard.reciver_buffer[0] == 0x69)
					{
						switch (cinterfacesmartcard.reciver_buffer[1])
						{
							case 0x82:
							{
								printf("Доступ не разрешен или ошибка предъявления транспортного кода \n : 69 82");
								break;
							}

							case 0x83:
							{
								printf("Ключ, пароль или транспортный код блокирован \n : 69 83");
								break;
							}
							case 0x85:
							{
								printf("Не выполнены условия использования команды (не выполнялись команды GET_CHALLENGE, GEN_SESSION_KEY, нет данных для GET_RESPONSE) \n : 69 85");
								break;
							}
							case 0x86:
							{
								printf("Команда не применима к данному типу файла \n : 69 86");
								break;
							}
							case 0xF0:
							{
								printf("Операция запрещена для данного ключа \n : 69 F0");
								break;
							}
							case 0xF2:
							{
								printf("В данной сессии не выработан диверсифицированный ключ \n : 69 F2");
								break;
							}

						}
					}
		}

	if((P2 == 0x00) && (P1 == 0x01))
		{

		int i=0;
		for(i=0; i < Lce; i++)
		{
			cinterfacesmartcard.transmit_buffer[i]=data[i];
		}
			/*
			cinterfacesmartcard.transmit_buffer[0]=data[0];
			cinterfacesmartcard.transmit_buffer[1]=data[1];
			cinterfacesmartcard.transmit_buffer[2]=data[2];
			cinterfacesmartcard.transmit_buffer[3]=data[3];
			cinterfacesmartcard.transmit_buffer[4]=data[4];
			cinterfacesmartcard.transmit_buffer[5]=data[5];
			cinterfacesmartcard.transmit_buffer[6]=data[6];
			cinterfacesmartcard.transmit_buffer[7]=data[7];
			*/

			write_byte(&ccomanda,(int)Lce,&cinterfacesmartcard,numer_sim); //8
			while(cinterfacesmartcard.ok==0){}
			cinterfacesmartcard.ok=0;
		}

	if((P2 == 0x00) && (P1 == 0x00))
	{

		cinterfacesmartcard.transmit_buffer[0]=data[0];
		cinterfacesmartcard.transmit_buffer[1]=data[1];
		cinterfacesmartcard.transmit_buffer[2]=data[2];
		cinterfacesmartcard.transmit_buffer[3]=data[3];
		cinterfacesmartcard.transmit_buffer[4]=data[4];
		cinterfacesmartcard.transmit_buffer[5]=data[5];
		cinterfacesmartcard.transmit_buffer[6]=data[6];
		cinterfacesmartcard.transmit_buffer[7]=data[7];

		write_byte(&ccomanda,8,&cinterfacesmartcard,numer_sim);
		while(cinterfacesmartcard.ok==0){}
		cinterfacesmartcard.ok=0;
	}

	if((P2 == 0x02) && (P1 == 0x01))
	{

		int i=0;
		for(i=0; i < Lce; i++)
		{
			cinterfacesmartcard.transmit_buffer[i]=data[i];
		}

		/*
			cinterfacesmartcard.transmit_buffer[0]=data[0];
			cinterfacesmartcard.transmit_buffer[1]=data[1];
			cinterfacesmartcard.transmit_buffer[2]=data[2];
			cinterfacesmartcard.transmit_buffer[3]=data[3];
			cinterfacesmartcard.transmit_buffer[4]=data[4];
			cinterfacesmartcard.transmit_buffer[5]=data[5];
			cinterfacesmartcard.transmit_buffer[6]=data[6];
			cinterfacesmartcard.transmit_buffer[7]=data[7];
			*/

			write_byte(&ccomanda,(int)Lce,&cinterfacesmartcard,numer_sim); //8
			while(cinterfacesmartcard.ok==0){}
			cinterfacesmartcard.ok=0;
	}

	if((P2 == 0x02) && (P1 == 0x0F))
	{

		printf("Завершающая посылка шифрования  \"Crypt\": \n");
				int i=0;
				for(i=0; i < Lce; i++)
				{
					cinterfacesmartcard.transmit_buffer[i]=data[i];
				}

				/*
				cinterfacesmartcard.transmit_buffer[0]=data[0];
				cinterfacesmartcard.transmit_buffer[1]=data[1];
				cinterfacesmartcard.transmit_buffer[2]=data[2];
				cinterfacesmartcard.transmit_buffer[3]=data[3];
				cinterfacesmartcard.transmit_buffer[4]=data[4];
				cinterfacesmartcard.transmit_buffer[5]=data[5];
				cinterfacesmartcard.transmit_buffer[6]=data[6];
				cinterfacesmartcard.transmit_buffer[7]=data[7];
				cinterfacesmartcard.transmit_buffer[8]=data[8];
				cinterfacesmartcard.transmit_buffer[9]=data[9];
				cinterfacesmartcard.transmit_buffer[10]=data[10];
				cinterfacesmartcard.transmit_buffer[11]=data[11];
				*/

				write_byte(&ccomanda,(int)Lce,&cinterfacesmartcard,numer_sim); //12
				while(cinterfacesmartcard.ok==0){}
				cinterfacesmartcard.ok=0;
	}

	if((P2 == 0x00) && (P1 == 0x0F))
		{
			printf("Завершающая посылка расшифрования  \"Crypt\": \n");
					int i=0;
					for(i=0; i < Lce; i++)
					{
						cinterfacesmartcard.transmit_buffer[i]=data[i];
					}
					/*
					cinterfacesmartcard.transmit_buffer[0]=data[0];
					cinterfacesmartcard.transmit_buffer[1]=data[1];
					cinterfacesmartcard.transmit_buffer[2]=data[2];
					cinterfacesmartcard.transmit_buffer[3]=data[3];
					cinterfacesmartcard.transmit_buffer[4]=data[4];
					cinterfacesmartcard.transmit_buffer[5]=data[5];
					cinterfacesmartcard.transmit_buffer[6]=data[6];
					cinterfacesmartcard.transmit_buffer[7]=data[7];
					cinterfacesmartcard.transmit_buffer[8]=data[8];
					cinterfacesmartcard.transmit_buffer[9]=data[9];
					cinterfacesmartcard.transmit_buffer[10]=data[10];
					cinterfacesmartcard.transmit_buffer[11]=data[11];
					*/

					write_byte(&ccomanda,(int)Lce,&cinterfacesmartcard,numer_sim); //12
					while(cinterfacesmartcard.ok==0){}
					cinterfacesmartcard.ok=0;
		}


	if(cinterfacesmartcard.reciver_buffer[0] == 0x61)
	{
		printf("End comand \"Crypt\" : Good = 61 0x%02hhx - количество байт \n", cinterfacesmartcard.reciver_buffer[1]);

		byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
		return byt;
	}


		if(cinterfacesmartcard.reciver_buffer[0] == 0x90)
		{
			if(cinterfacesmartcard.reciver_buffer[1] == 0x00)
			{
				printf("End comand \"Crypt\" : Good = 90 00 \n");
			}
		}
		else
		{
			if(cinterfacesmartcard.reciver_buffer[0] == 0x63)
			{
				printf("Ошибка аутентификации или предъявления пароля: %d - число оставшихся попыток \n",cinterfacesmartcard.reciver_buffer[1]);
			}

			if(cinterfacesmartcard.reciver_buffer[0] == 0x69)
			{
				switch (cinterfacesmartcard.reciver_buffer[1])
				{
					case 0x82:
					{
						printf("Доступ не разрешен или ошибка предъявления транспортного кода \n : 69 82");
						break;
					}

					case 0x83:
					{
						printf("Ключ, пароль или транспортный код блокирован \n : 69 83");
						break;
					}
					case 0x85:
					{
						printf("Не выполнены условия использования команды (не выполнялись команды GET_CHALLENGE, GEN_SESSION_KEY, нет данных для GET_RESPONSE) \n : 69 85");
						break;
					}
					case 0x86:
					{
						printf("Команда не применима к данному типу файла \n : 69 86");
						break;
					}
					case 0xF0:
					{
						printf("Операция запрещена для данного ключа \n : 69 F0");
						break;
					}
					case 0xF2:
					{
						printf("В данной сессии не выработан диверсифицированный ключ \n : 69 F2");
						break;
					}

				}
			}

											if(cinterfacesmartcard.reciver_buffer[0] == 0x6A)
											{
												switch (cinterfacesmartcard.reciver_buffer[1])
												{
													case 0x83:
													{
														printf("Файл пуст \n : 6A 83");
														break;
													}
													case 0x86:
													{
														printf("Не верные параметры Р1 Р2 \n : 6A 86");
														break;
													}
													case 0x84:
													{
														printf("Не достаточно места для создания файла \n : 6A 84");
														break;
													}
													case 0x80:
													{
														printf("Неприемлемое значение поля данных (файл с указанным идентификатором уже присутствует) \n : 6A 80");
														break;
													}
												}
											}

											if(cinterfacesmartcard.reciver_buffer[0] == 0xff)
											{
												printf("Error comand \"Crypt\" : ff (Неизвестная команда, карта заблокированна для выполнения команд) \n");
											}
		}
		printf("=========================\n");


		byt[0]=cinterfacesmartcard.reciver_buffer[1]; //
		byt[1]=cinterfacesmartcard.reciver_buffer[2]; //
		byt[2]=cinterfacesmartcard.reciver_buffer[3]; //
		byt[3]=0; //
		return byt;

}

unsigned transmit_crypt(char* data,int size, int message)
{

	//Разбиваем посылку на под посылки

	int count_message = 0;

	int count_message_last = 0;

	char* data_messege;


	switch (message) {

		case 1:
		{
			data_messege = malloc(8);

			count_message = size / 8;

			if((size % 8) > 0)
			{
				count_message_last = size - (count_message*8);
			}

			break;
		}
		case 2:
		{
			data_messege = malloc(16);

			count_message = size / 16;

			if((size % 16) > 0)
			{
				count_message_last = size - (count_message*16);
			}


			break;
		}

		case 3:
		{
			data_messege = malloc(24);

			count_message = size / 24;

			if((size % 24) > 0)
			{
				count_message_last = size - (count_message*24);
			}

			break;
		}
		case 4:
		{
			data_messege = malloc(32);

			count_message = size / 32;

			if((size % 32) > 0)
			{
				count_message_last = size - (count_message*32);
			}

			break;
		}

		case 5:
		{
			data_messege = malloc(40);

			count_message = size / 40;

			if((size % 40) > 0)
			{
				count_message_last = size - (count_message*40);
			}

			break;
		}
		case 6:
		{
			data_messege = malloc(48);

			count_message = size / 48;

			if((size % 48) > 0)
			{
				count_message_last = size - (count_message*48);
			}

			break;
		}

		case 7:
		{
			data_messege = malloc(56);

			count_message = size / 56;


			if((size % 56) > 0)
			{
				count_message_last = size - (count_message*56);
			}

			break;
		}


	}


	int i=0;
	int j=0;
	int k=0;

	printf("Переход на KF файл базовой станции: \n");
	select(0xFF,0x13,0);
	clean_cInterfaceSmartcard(&cinterfacesmartcard);

	char* get_data = malloc(1);
	char * data_sinhr = malloc(8);

	get_data = Crypt(get_data,0x00,0x02,0x08,0); // открытие сеанса зашифрования
	clean_cInterfaceSmartcard(&cinterfacesmartcard);



	printf("get_data = %d \n",get_data);

	data_sinhr = get_response(0x08,0); // получить синхропассылку для расшифрования
	clean_cInterfaceSmartcard(&cinterfacesmartcard);

	printf("data_sinhr  : \n");
	for(i=0; i < 8;i++)
	{
		printf("0x%02hhx \n",data_sinhr[i]);
	}


	printf("Переход на KF файл клиента: \n");
	select(0xFF,0x13,1);
	clean_cInterfaceSmartcard(&cinterfacesmartcard);

	data_sinhr = Crypt(data_sinhr,0x00,0x00,0x08,1); // открытие сеанса расшифрования

	char* get_data_1 = malloc(1);





	//////////////////////////////////////////////////

	switch (message) {

			case 1:
			{


				if( size <= 0x08 )
				{

					for(j=0; j < size; j++)
					{
						data_messege[j] = data[j];
					}



					////////////////////////////

							printf("data_messege  : \n");
							for(k=0; k < size;k++)
							{
								printf("%d ) 0x%02hhx \n",k,data_messege[k]);
							}

							get_data_1 = Crypt(data_messege,0x0F,0x02,size,0);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							char* data3 = malloc(size);
							data3 = get_response(size,0);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							printf("data3  : \n");
							for(k=0; k < size;k++)
							{
								printf("%d ) 0x%02hhx \n",k,data3[k]);
							}

							get_data_1 = Crypt(data3,0x0F,0x00,size,1);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							char* data4 = malloc(size);
							data4 = get_response(size,1);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							printf("data4  : \n");
							for(k=0; k < size;k++)
							{
								printf("%d ) 0x%02hhx \n",k,data4[k]);
							}


							return 0;
				}

				break;
			}
			case 2:
			{


				if( size <= 0x10 )
				{

					for(j=0; j < size; j++)
					{
						data_messege[j] = data[j];
					}



					////////////////////////////

							printf("data_messege  : \n");
							for(k=0; k < size;k++)
							{
								printf("%d ) 0x%02hhx \n",k,data_messege[k]);
							}

							get_data_1 = Crypt(data_messege,0x0F,0x02,size,0);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							char* data3 = malloc(size);
							data3 = get_response(size,0);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							printf("data3  : \n");
							for(k=0; k < size;k++)
							{
								printf("%d ) 0x%02hhx \n",k,data3[k]);
							}

							get_data_1 = Crypt(data3,0x0F,0x00,size,1);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							char* data4 = malloc(size);
							data4 = get_response(size,1);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							printf("data4  : \n");
							for(k=0; k < size;k++)
							{
								printf("%d ) 0x%02hhx \n",k,data4[k]);
							}


							return 0;
				}


				break;
			}

			case 3:
			{


				if( size <= 0x18 )
				{

					for(j=0; j < size; j++)
					{
						data_messege[j] = data[j];
					}



					////////////////////////////

							printf("data_messege  : \n");
							for(k=0; k < size;k++)
							{
								printf("%d ) 0x%02hhx \n",k,data_messege[k]);
							}

							get_data_1 = Crypt(data_messege,0x0F,0x02,size,0);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							char* data3 = malloc(size);
							data3 = get_response(size,0);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							printf("data3  : \n");
							for(k=0; k < size;k++)
							{
								printf("%d ) 0x%02hhx \n",k,data3[k]);
							}

							get_data_1 = Crypt(data3,0x0F,0x00,size,1);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							char* data4 = malloc(size);
							data4 = get_response(size,1);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							printf("data4  : \n");
							for(k=0; k < size;k++)
							{
								printf("%d ) 0x%02hhx \n",k,data4[k]);
							}


							return 0;
				}

				break;
			}
			case 4:
			{


				if( size <= 0x20 )
				{

					for(j=0; j < size; j++)
					{
						data_messege[j] = data[j];
					}



					////////////////////////////

							printf("data_messege  : \n");
							for(k=0; k < size;k++)
							{
								printf("%d ) 0x%02hhx \n",k,data_messege[k]);
							}

							get_data_1 = Crypt(data_messege,0x0F,0x02,size,0);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							char* data3 = malloc(size);
							data3 = get_response(size,0);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							printf("data3  : \n");
							for(k=0; k < size;k++)
							{
								printf("%d ) 0x%02hhx \n",k,data3[k]);
							}

							get_data_1 = Crypt(data3,0x0F,0x00,size,1);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							char* data4 = malloc(size);
							data4 = get_response(size,1);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							printf("data4  : \n");
							for(k=0; k < size;k++)
							{
								printf("%d ) 0x%02hhx \n",k,data4[k]);
							}


							return 0;
				}

				break;
			}

			case 5:
			{


				if( size <= 0x28 )
				{

					for(j=0; j < size; j++)
					{
						data_messege[j] = data[j];
					}



					////////////////////////////

							printf("data_messege  : \n");
							for(k=0; k < size;k++)
							{
								printf("%d ) 0x%02hhx \n",k,data_messege[k]);
							}

							get_data_1 = Crypt(data_messege,0x0F,0x02,size,0);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							char* data3 = malloc(size);
							data3 = get_response(size,0);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							printf("data3  : \n");
							for(k=0; k < size;k++)
							{
								printf("%d ) 0x%02hhx \n",k,data3[k]);
							}

							get_data_1 = Crypt(data3,0x0F,0x00,size,1);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							char* data4 = malloc(size);
							data4 = get_response(size,1);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							printf("data4  : \n");
							for(k=0; k < size;k++)
							{
								printf("%d ) 0x%02hhx \n",k,data4[k]);
							}


							return 0;
				}

						/////////////////


				break;
			}
			case 6:
			{


				if( size <= 0x30 )
				{

					for(j=0; j < size; j++)
					{
						data_messege[j] = data[j];
					}



					////////////////////////////

							printf("data_messege  : \n");
							for(k=0; k < size;k++)
							{
								printf("%d ) 0x%02hhx \n",k,data_messege[k]);
							}

							get_data_1 = Crypt(data_messege,0x0F,0x02,size,0);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							char* data3 = malloc(size);
							data3 = get_response(size,0);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							printf("data3  : \n");
							for(k=0; k < size;k++)
							{
								printf("%d ) 0x%02hhx \n",k,data3[k]);
							}

							get_data_1 = Crypt(data3,0x0F,0x00,size,1);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							char* data4 = malloc(size);
							data4 = get_response(size,1);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							printf("data4  : \n");
							for(k=0; k < size;k++)
							{
								printf("%d ) 0x%02hhx \n",k,data4[k]);
							}


							return 0;
				}

						/////////////////


				break;
			}

			case 7:
			{


				if( size <= 0x38 )
				{

					for(j=0; j < size; j++)
					{
						data_messege[j] = data[j];
					}



					////////////////////////////

							printf("data_messege  : \n");
							for(k=0; k < size;k++)
							{
								printf("%d ) 0x%02hhx \n",k,data_messege[k]);
							}

							get_data_1 = Crypt(data_messege,0x0F,0x02,size,0);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							char* data3 = malloc(size);
							data3 = get_response(0x38,0);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							printf("data3  : \n");
							for(k=0; k < size;k++)
							{
								printf("%d ) 0x%02hhx \n",k,data3[k]);
							}

							get_data_1 = Crypt(data3,0x0F,0x00,size,1);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							char* data4 = malloc(size);
							data4 = get_response(size,1);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							printf("data4  : \n");
							for(k=0; k < size;k++)
							{
								printf("%d ) 0x%02hhx \n",k,data4[k]);
							}


							return 0;

						/////////////////


				}


				break;
			}


		}

	/////////////////////////////////////////////////


	//////////////////////////////////////////////////
	if(count_message_last > 0)
	{

	for(i = 0 ; i < count_message;i++)
	{

		switch (message)
		{

				case 1:
				{

					for(j=0; j < 8; j++)
					{
						if(i==0)
						{
							data_messege[j] = data[j];
						}
						else
						{
							if(j==0)
							{
								data_messege[j] = data[i*8];
							}
							else
							{
								data_messege[j] = data[(i*8)+j];
							}
						}

					}

					////////////////////////////

					printf("data_messege  : \n");
					for(k=0; k < 8;k++)
					{
						printf("%d ) 0x%02hhx \n",k,data_messege[k]);
					}

					get_data_1 = Crypt(data_messege,0x01,0x02,0x08,0); // зашифрования порции данных от EF
					clean_cInterfaceSmartcard(&cinterfacesmartcard);

					char* data1 = malloc(8);

					data1 = get_response(0x08,0);
					clean_cInterfaceSmartcard(&cinterfacesmartcard);

					printf("data1  : \n");
					for(k=0; k < 8;k++)
					{
						printf("%d ) 0x%02hhx \n",k,data1[k]);
					}


					get_data_1 = Crypt(data1,0x01,0x00,0x08,1);
					clean_cInterfaceSmartcard(&cinterfacesmartcard);

					char* data2 = malloc(8);
					data2 = get_response(0x08,1);
					clean_cInterfaceSmartcard(&cinterfacesmartcard);

					printf("data2  : \n");
					for(k=0; k < 8;k++)
					{
						printf("%d ) 0x%02hhx \n",k,data2[k]);
					}

					////////////////////////////

					break;
				}

				case 2:
				{

					for(j=0; j < 16; j++)
					{
						if(i==0)
						{
							data_messege[j] = data[j];
						}
						else
						{
							if(j==0)
							{
								data_messege[j] = data[i*16];
							}
							else
							{
								data_messege[j] = data[(i*16)+j];
							}
						}

					}


					////////////////////////////

					printf("data_messege  : \n");
					for(k=0; k < 16;k++)
					{
						printf("%d ) 0x%02hhx \n",k,data_messege[k]);
					}

					get_data_1 = Crypt(data_messege,0x01,0x02,0x10,0); // зашифрования порции данных от EF
					clean_cInterfaceSmartcard(&cinterfacesmartcard);

					char* data1 = malloc(16);

					data1 = get_response(0x10,0);
					clean_cInterfaceSmartcard(&cinterfacesmartcard);

					printf("data1  : \n");
					for(k=0; k < 16;k++)
					{
						printf("%d ) 0x%02hhx \n",k,data1[k]);
					}


					get_data_1 = Crypt(data1,0x01,0x00,0x10,1);
					clean_cInterfaceSmartcard(&cinterfacesmartcard);

					char* data2 = malloc(16);
					data2 = get_response(0x10,1);
					clean_cInterfaceSmartcard(&cinterfacesmartcard);

					printf("data2  : \n");
					for(k=0; k < 16;k++)
					{
						printf("%d ) 0x%02hhx \n",k,data2[k]);
					}

					////////////////////////////


					break;
				}


				case 3:
				{

					for(j=0; j < 24; j++)
					{
						if(i==0)
						{
							data_messege[j] = data[j];
						}
						else
						{
							if(j==0)
							{
								data_messege[j] = data[i*24];
							}
							else
							{
								data_messege[j] = data[(i*24)+j];
							}
						}

					}


					////////////////////////////

					printf("data_messege  : \n");
					for(k=0; k < 24;k++)
					{
						printf("%d ) 0x%02hhx \n",k,data_messege[k]);
					}

					get_data_1 = Crypt(data_messege,0x01,0x02,0x18,0); // зашифрования порции данных от EF
					clean_cInterfaceSmartcard(&cinterfacesmartcard);

					char* data1 = malloc(24);

					data1 = get_response(0x18,0);
					clean_cInterfaceSmartcard(&cinterfacesmartcard);

					printf("data1  : \n");
					for(k=0; k < 24;k++)
					{
						printf("%d ) 0x%02hhx \n",k,data1[k]);
					}


					get_data_1 = Crypt(data1,0x01,0x00,0x18,1);
					clean_cInterfaceSmartcard(&cinterfacesmartcard);

					char* data2 = malloc(24);
					data2 = get_response(0x18,1);
					clean_cInterfaceSmartcard(&cinterfacesmartcard);

					printf("data2  : \n");
					for(k=0; k < 24;k++)
					{
						printf("%d ) 0x%02hhx \n",k,data2[k]);
					}

					////////////////////////////


					break;
				}


				case 4:
				{

					for(j=0; j < 32; j++)
					{
						if(i==0)
						{
							data_messege[j] = data[j];
						}
						else
						{
							if(j==0)
							{
								data_messege[j] = data[i*32];
							}
							else
							{
								data_messege[j] = data[(i*32)+j];
							}
						}

					}

					////////////////////////////

										printf("data_messege  : \n");
										for(k=0; k < 32;k++)
										{
											printf("%d ) 0x%02hhx \n",k,data_messege[k]);
										}

										get_data_1 = Crypt(data_messege,0x01,0x02,0x20,0); // зашифрования порции данных от EF
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										char* data1 = malloc(32);

										data1 = get_response(0x20,0);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										printf("data1  : \n");
										for(k=0; k < 32;k++)
										{
											printf("%d ) 0x%02hhx \n",k,data1[k]);
										}


										get_data_1 = Crypt(data1,0x01,0x00,0x20,1);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										char* data2 = malloc(32);
										data2 = get_response(0x20,1);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										printf("data2  : \n");
										for(k=0; k < 32;k++)
										{
											printf("%d ) 0x%02hhx \n",k,data2[k]);
										}

										////////////////////////////

					break;
				}

				case 5:
				{

					for(j=0; j < 40; j++)
					{
						if(i==0)
						{
							data_messege[j] = data[j];
						}
						else
						{
							if(j==0)
							{
								data_messege[j] = data[i*40];
							}
							else
							{
								data_messege[j] = data[(i*40)+j];
							}
						}

					}


					////////////////////////////

										printf("data_messege  : \n");
										for(k=0; k < 40;k++)
										{
											printf("%d ) 0x%02hhx \n",k,data_messege[k]);
										}

										get_data_1 = Crypt(data_messege,0x01,0x02,0x28,0); // зашифрования порции данных от EF
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										char* data1 = malloc(40);

										data1 = get_response(0x28,0);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										printf("data1  : \n");
										for(k=0; k < 40;k++)
										{
											printf("%d ) 0x%02hhx \n",k,data1[k]);
										}


										get_data_1 = Crypt(data1,0x01,0x00,0x28,1);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										char* data2 = malloc(40);
										data2 = get_response(0x28,1);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										printf("data2  : \n");
										for(k=0; k < 40;k++)
										{
											printf("%d ) 0x%02hhx \n",k,data2[k]);
										}

										////////////////////////////


					break;
				}

				case 6:
				{

					for(j=0; j < 48; j++)
					{
						if(i==0)
						{
							data_messege[j] = data[j];
						}
						else
						{
							if(j==0)
							{
								data_messege[j] = data[i*48];
							}
							else
							{
								data_messege[j] = data[(i*48)+j];
							}
						}

					}


					////////////////////////////

										printf("data_messege  : \n");
										for(k=0; k < 48;k++)
										{
											printf("%d ) 0x%02hhx \n",k,data_messege[k]);
										}

										get_data_1 = Crypt(data_messege,0x01,0x02,0x30,0); // зашифрования порции данных от EF
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										char* data1 = malloc(48);

										data1 = get_response(0x30,0);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										printf("data1  : \n");
										for(k=0; k < 48;k++)
										{
											printf("%d ) 0x%02hhx \n",k,data1[k]);
										}


										get_data_1 = Crypt(data1,0x01,0x00,0x30,1);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										char* data2 = malloc(48);
										data2 = get_response(0x30,1);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										printf("data2  : \n");
										for(k=0; k < 48;k++)
										{
											printf("%d ) 0x%02hhx \n",k,data2[k]);
										}

										////////////////////////////


					break;
				}

				case 7:
				{

					for(j=0; j < 56; j++)
					{
						if(i==0)
						{
							data_messege[j] = data[j];
						}
						else
						{
							if(j==0)
							{
								data_messege[j] = data[i*56];
							}
							else
							{
								data_messege[j] = data[(i*56)+j];
							}
						}

					}


					////////////////////////////

										printf("data_messege  : \n");
										for(k=0; k < 56;k++)
										{
											printf("%d ) 0x%02hhx \n",k,data_messege[k]);
										}

										get_data_1 = Crypt(data_messege,0x01,0x02,0x38,0); // зашифрования порции данных от EF
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										char* data1 = malloc(56);

										data1 = get_response(0x38,0);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										printf("data1  : \n");
										for(k=0; k < 56;k++)
										{
											printf("%d ) 0x%02hhx \n",k,data1[k]);
										}


										get_data_1 = Crypt(data1,0x01,0x00,0x38,1);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										char* data2 = malloc(56);
										data2 = get_response(0x38,1);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										printf("data2  : \n");
										for(k=0; k < 56;k++)
										{
											printf("%d ) 0x%02hhx \n",k,data2[k]);
										}

										////////////////////////////


					break;
				}

		}

		//Дописать на конец или додумать сколько зацикливать чтоб закрыть сеанс связи
		//Дописать код который нужен для закрытия сеанса связи при маленькой посылке

	}


		///////////////

		for(j=0; j < count_message_last; j++)
		{
			if(j==0)
			{
				data_messege[j] = data[(size-count_message_last)];
			}
			else
			{
				data_messege[j] = data[(size-count_message_last)+j];
			}
		}

		////////////////////////////

		printf("data_messege  : \n");
		for(i=0; i < count_message_last;i++)
		{
			printf("0x%02hhx \n",data_messege[i]);
		}


		char size_count_message_last;

		sprintf(count_message_last,0x01,"0x%02hhx",size_count_message_last);

		printf(" count_message_last = 0x%02hhx \n",count_message_last+48);
		printf(" size_count_message_last = 0x%02hhx \n",size_count_message_last);


		get_data_1 = Crypt(data_messege,0x0F,0x02,count_message_last,0);
		clean_cInterfaceSmartcard(&cinterfacesmartcard);

		char* data3 = malloc(count_message_last);
		data3 = get_response(count_message_last,0);
		clean_cInterfaceSmartcard(&cinterfacesmartcard);

		printf("data3  : \n");
		for(i=0; i < count_message_last;i++)
		{
			printf("0x%02hhx \n",data3[i]);
		}

		get_data_1 = Crypt(data3,0x0F,0x00,count_message_last,1);
		clean_cInterfaceSmartcard(&cinterfacesmartcard);

		char* data4 = malloc(count_message_last);
		data4 = get_response(count_message_last,1);
		clean_cInterfaceSmartcard(&cinterfacesmartcard);

		printf("data4  : \n");
		for(i=0; i < count_message_last;i++)
		{
			printf("0x%02hhx \n",data4[i]);
		}


	/////////////////

	}
	else
	{

		//////////////


		for(i = 0 ; i < count_message;i++)
			{

				switch (message)
				{

						case 1:
						{

							for(j=0; j < 8; j++)
							{
								if(i==0)
								{
									data_messege[j] = data[j];
								}
								else
								{
									if(j==0)
									{
										data_messege[j] = data[i*8];
									}
									else
									{
										data_messege[j] = data[(i*8)+j];
									}
								}

							}

							////////////////////////////

							if(i != count_message-1)
							{

							printf("data_messege  : \n");
							for(k=0; k < 8;k++)
							{
								printf("0x%02hhx \n",data_messege[k]);
							}

							get_data_1 = Crypt(data_messege,0x01,0x02,0x08,0); // зашифрования порции данных от EF
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							char* data1 = malloc(8);

							data1 = get_response(0x08,0);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							printf("data1  : \n");
							for(k=0; k < 8;k++)
							{
								printf("0x%02hhx \n",data1[k]);
							}


							get_data_1 = Crypt(data1,0x01,0x00,0x08,1);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							char* data2 = malloc(8);
							data2 = get_response(0x08,1);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							printf("data2  : \n");
							for(k=0; k < 8;k++)
							{
								printf("0x%02hhx \n",data2[k]);
							}

							}
							else
							{

								////////////////////////////

										printf("data_messege  : \n");
										for(k=0; k < 8;k++)
										{
											printf("0x%02hhx \n",data_messege[k]);
										}

										get_data_1 = Crypt(data_messege,0x0F,0x02,0x08,0);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										char* data3 = malloc(8);
										data3 = get_response(0x08,0);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										printf("data3  : \n");
										for(k=0; k < 8;k++)
										{
											printf("0x%02hhx \n",data3[k]);
										}

										get_data_1 = Crypt(data3,0x0F,0x00,0x08,1);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										char* data4 = malloc(8);
										data4 = get_response(0x08,1);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										printf("data4  : \n");
										for(k=0; k < 8;k++)
										{
											printf("0x%02hhx \n",data4[k]);
										}


									/////////////////

							}

							////////////////////////////

							break;
						}

						case 2:
						{

							for(j=0; j < 16; j++)
							{
								if(i==0)
								{
									data_messege[j] = data[j];
								}
								else
								{
									if(j==0)
									{
										data_messege[j] = data[i*16];
									}
									else
									{
										data_messege[j] = data[(i*16)+j];
									}
								}

							}

							if( i != count_message-1)
							{
							////////////////////////////

							printf("data_messege  : \n");
							for(k=0; k < 16;k++)
							{
								printf("0x%02hhx \n",data_messege[k]);
							}

							get_data_1 = Crypt(data_messege,0x01,0x02,0x10,0); // зашифрования порции данных от EF
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							char* data1 = malloc(16);

							data1 = get_response(0x10,0);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							printf("data1  : \n");
							for(k=0; k < 16;k++)
							{
								printf("0x%02hhx \n",data1[k]);
							}


							get_data_1 = Crypt(data1,0x01,0x00,0x10,1);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							char* data2 = malloc(16);
							data2 = get_response(0x10,1);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							printf("data2  : \n");
							for(k=0; k < 16;k++)
							{
								printf("0x%02hhx \n",data2[k]);
							}

							////////////////////////////
							}
							else
							{


								////////////////////////////

										printf("data_messege  : \n");
										for(k=0; k < 16;k++)
										{
											printf("0x%02hhx \n",data_messege[k]);
										}

										get_data_1 = Crypt(data_messege,0x0F,0x02,0x10,0);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										char* data3 = malloc(16);
										data3 = get_response(0x10,0);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										printf("data3  : \n");
										for(k=0; k < 16;k++)
										{
											printf("0x%02hhx \n",data3[k]);
										}

										get_data_1 = Crypt(data3,0x0F,0x00,0x10,1);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										char* data4 = malloc(16);
										data4 = get_response(0x10,1);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										printf("data4  : \n");
										for(k=0; k < 16;k++)
										{
											printf("0x%02hhx \n",data4[k]);
										}


									/////////////////


							}


							break;
						}


						case 3:
						{

							for(j=0; j < 24; j++)
							{
								if(i==0)
								{
									data_messege[j] = data[j];
								}
								else
								{
									if(j==0)
									{
										data_messege[j] = data[i*24];
									}
									else
									{
										data_messege[j] = data[(i*24)+j];
									}
								}

							}

							if( i != count_message-1)
							{
							////////////////////////////

							printf("data_messege  : \n");
							for(k=0; k < 24;k++)
							{
								printf("0x%02hhx \n",data_messege[k]);
							}

							get_data_1 = Crypt(data_messege,0x01,0x02,0x18,0); // зашифрования порции данных от EF
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							char* data1 = malloc(24);

							data1 = get_response(0x18,0);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							printf("data1  : \n");
							for(k=0; k < 24;k++)
							{
								printf("0x%02hhx \n",data1[k]);
							}


							get_data_1 = Crypt(data1,0x01,0x00,0x18,1);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							char* data2 = malloc(24);
							data2 = get_response(0x18,1);
							clean_cInterfaceSmartcard(&cinterfacesmartcard);

							printf("data2  : \n");
							for(k=0; k < 24;k++)
							{
								printf("0x%02hhx \n",data2[k]);
							}

							////////////////////////////
							}
							else
							{


								////////////////////////////

										printf("data_messege  : \n");
										for(k=0; k < 24;k++)
										{
											printf("0x%02hhx \n",data_messege[k]);
										}

										get_data_1 = Crypt(data_messege,0x0F,0x02,0x18,0);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										char* data3 = malloc(24);
										data3 = get_response(0x18,0);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										printf("data3  : \n");
										for(k=0; k < 24;k++)
										{
											printf("0x%02hhx \n",data3[k]);
										}

										get_data_1 = Crypt(data3,0x0F,0x00,0x18,1);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										char* data4 = malloc(24);
										data4 = get_response(0x18,1);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										printf("data4  : \n");
										for(k=0; k < 24;k++)
										{
											printf("0x%02hhx \n",data4[k]);
										}


									/////////////////

							}


							break;
						}


						case 4:
						{

							for(j=0; j < 32; j++)
							{
								if(i==0)
								{
									data_messege[j] = data[j];
								}
								else
								{
									if(j==0)
									{
										data_messege[j] = data[i*32];
									}
									else
									{
										data_messege[j] = data[(i*32)+j];
									}
								}

							}

							if(i != count_message-1)
							{
							////////////////////////////

												printf("data_messege  : \n");
												for(k=0; k < 32;k++)
												{
													printf("0x%02hhx \n",data_messege[k]);
												}

												get_data_1 = Crypt(data_messege,0x01,0x02,0x20,0); // зашифрования порции данных от EF
												clean_cInterfaceSmartcard(&cinterfacesmartcard);

												char* data1 = malloc(32);

												data1 = get_response(0x20,0);
												clean_cInterfaceSmartcard(&cinterfacesmartcard);

												printf("data1  : \n");
												for(k=0; k < 32;k++)
												{
													printf("0x%02hhx \n",data1[k]);
												}


												get_data_1 = Crypt(data1,0x01,0x00,0x20,1);
												clean_cInterfaceSmartcard(&cinterfacesmartcard);

												char* data2 = malloc(32);
												data2 = get_response(0x20,1);
												clean_cInterfaceSmartcard(&cinterfacesmartcard);

												printf("data2  : \n");
												for(k=0; k < 32;k++)
												{
													printf("0x%02hhx \n",data2[k]);
												}

												////////////////////////////
							}
							else
							{


								////////////////////////////

										printf("data_messege  : \n");
										for(k=0; k < 32;k++)
										{
											printf("0x%02hhx \n",data_messege[k]);
										}

										get_data_1 = Crypt(data_messege,0x0F,0x02,0x20,0);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										char* data3 = malloc(32);
										data3 = get_response(0x20,0);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										printf("data3  : \n");
										for(k=0; k < 32;k++)
										{
											printf("0x%02hhx \n",data3[k]);
										}

										get_data_1 = Crypt(data3,0x0F,0x00,0x20,1);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										char* data4 = malloc(32);
										data4 = get_response(0x20,1);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										printf("data4  : \n");
										for(k=0; k < 32;k++)
										{
											printf("0x%02hhx \n",data4[k]);
										}


									/////////////////

							}

							break;
						}

						case 5:
						{

							for(j=0; j < 40; j++)
							{
								if(i==0)
								{
									data_messege[j] = data[j];
								}
								else
								{
									if(j==0)
									{
										data_messege[j] = data[i*40];
									}
									else
									{
										data_messege[j] = data[(i*40)+j];
									}
								}

							}

							if( i != count_message-1)
							{
							////////////////////////////

												printf("data_messege  : \n");
												for(k=0; k < 40;k++)
												{
													printf("0x%02hhx \n",data_messege[k]);
												}

												get_data_1 = Crypt(data_messege,0x01,0x02,0x28,0); // зашифрования порции данных от EF
												clean_cInterfaceSmartcard(&cinterfacesmartcard);

												char* data1 = malloc(40);

												data1 = get_response(0x28,0);
												clean_cInterfaceSmartcard(&cinterfacesmartcard);

												printf("data1  : \n");
												for(k=0; k < 40;k++)
												{
													printf("0x%02hhx \n",data1[k]);
												}


												get_data_1 = Crypt(data1,0x01,0x00,0x28,1);
												clean_cInterfaceSmartcard(&cinterfacesmartcard);

												char* data2 = malloc(40);
												data2 = get_response(0x28,1);
												clean_cInterfaceSmartcard(&cinterfacesmartcard);

												printf("data2  : \n");
												for(k=0; k < 40;k++)
												{
													printf("0x%02hhx \n",data2[k]);
												}

												////////////////////////////
							}
							else
							{


								////////////////////////////

										printf("data_messege  : \n");
										for(k=0; k < 40;k++)
										{
											printf("0x%02hhx \n",data_messege[k]);
										}

										get_data_1 = Crypt(data_messege,0x0F,0x02,0x28,0);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										char* data3 = malloc(40);
										data3 = get_response(0x28,0);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										printf("data3  : \n");
										for(k=0; k < 40;k++)
										{
											printf("0x%02hhx \n",data3[k]);
										}

										get_data_1 = Crypt(data3,0x0F,0x00,0x28,1);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										char* data4 = malloc(40);
										data4 = get_response(0x28,1);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										printf("data4  : \n");
										for(k=0; k < 40;k++)
										{
											printf("0x%02hhx \n",data4[k]);
										}


									/////////////////

							}


							break;
						}

						case 6:
						{

							for(j=0; j < 48; j++)
							{
								if(i==0)
								{
									data_messege[j] = data[j];
								}
								else
								{
									if(j==0)
									{
										data_messege[j] = data[i*48];
									}
									else
									{
										data_messege[j] = data[(i*48)+j];
									}
								}

							}

							if( i != count_message-1)
							{
							////////////////////////////

												printf("data_messege  : \n");
												for(k=0; k < 48;k++)
												{
													printf("0x%02hhx \n",data_messege[k]);
												}

												get_data_1 = Crypt(data_messege,0x01,0x02,0x30,0); // зашифрования порции данных от EF
												clean_cInterfaceSmartcard(&cinterfacesmartcard);

												char* data1 = malloc(48);

												data1 = get_response(0x30,0);
												clean_cInterfaceSmartcard(&cinterfacesmartcard);

												printf("data1  : \n");
												for(k=0; k < 48;k++)
												{
													printf("0x%02hhx \n",data1[k]);
												}


												get_data_1 = Crypt(data1,0x01,0x00,0x30,1);
												clean_cInterfaceSmartcard(&cinterfacesmartcard);

												char* data2 = malloc(48);
												data2 = get_response(0x30,1);
												clean_cInterfaceSmartcard(&cinterfacesmartcard);

												printf("data2  : \n");
												for(k=0; k < 48;k++)
												{
													printf("0x%02hhx \n",data2[k]);
												}

												////////////////////////////
							}
							else
							{


								////////////////////////////

										printf("data_messege  : \n");
										for(k=0; k < 48;k++)
										{
											printf("0x%02hhx \n",data_messege[k]);
										}

										get_data_1 = Crypt(data_messege,0x0F,0x02,0x30,0);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										char* data3 = malloc(48);
										data3 = get_response(0x30,0);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										printf("data3  : \n");
										for(k=0; k < 48;k++)
										{
											printf("0x%02hhx \n",data3[k]);
										}

										get_data_1 = Crypt(data3,0x0F,0x00,0x30,1);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										char* data4 = malloc(48);
										data4 = get_response(0x30,1);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										printf("data4  : \n");
										for(k=0; k < 48;k++)
										{
											printf("0x%02hhx \n",data4[k]);
										}


									/////////////////

							}


							break;
						}

						case 7:
						{

							for(j=0; j < 56; j++)
							{
								if(i==0)
								{
									data_messege[j] = data[j];
								}
								else
								{
									if(j==0)
									{
										data_messege[j] = data[i*56];
									}
									else
									{
										data_messege[j] = data[(i*56)+j];
									}
								}

							}


							if( i != count_message-1)
							{
							////////////////////////////

												printf("data_messege  : \n");
												for(k=0; k < 56;k++)
												{
													printf("0x%02hhx \n",data_messege[k]);
												}

												get_data_1 = Crypt(data_messege,0x01,0x02,0x38,0); // зашифрования порции данных от EF
												clean_cInterfaceSmartcard(&cinterfacesmartcard);

												char* data1 = malloc(56);

												data1 = get_response(0x38,0);
												clean_cInterfaceSmartcard(&cinterfacesmartcard);

												printf("data1  : \n");
												for(k=0; k < 56;k++)
												{
													printf("0x%02hhx \n",data1[k]);
												}


												get_data_1 = Crypt(data1,0x01,0x00,0x38,1);
												clean_cInterfaceSmartcard(&cinterfacesmartcard);

												char* data2 = malloc(56);
												data2 = get_response(0x38,1);
												clean_cInterfaceSmartcard(&cinterfacesmartcard);

												printf("data2  : \n");
												for(k=0; k < 56;k++)
												{
													printf("0x%02hhx \n",data2[k]);
												}

												////////////////////////////
							}
							else
							{


								////////////////////////////

										printf("data_messege  : \n");
										for(k=0; k < 56;k++)
										{
											printf("0x%02hhx \n",data_messege[k]);
										}

										get_data_1 = Crypt(data_messege,0x0F,0x02,0x38,0);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										char* data3 = malloc(56);
										data3 = get_response(0x38,0);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										printf("data3  : \n");
										for(k=0; k < 56;k++)
										{
											printf("0x%02hhx \n",data3[k]);
										}

										get_data_1 = Crypt(data3,0x0F,0x00,0x38,1);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										char* data4 = malloc(56);
										data4 = get_response(0x38,1);
										clean_cInterfaceSmartcard(&cinterfacesmartcard);

										printf("data4  : \n");
										for(k=0; k < 56;k++)
										{
											printf("0x%02hhx \n",data4[k]);
										}


									/////////////////

							}


							break;
						}

				}

				//Дописать на конец или додумать сколько зацикливать чтоб закрыть сеанс связи
				//Дописать код который нужен для закрытия сеанса связи при маленькой посылке

			}


		///////////////
	}

	return 0;
}
